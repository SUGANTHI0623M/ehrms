...
const EmployeeDashboard = () => {
  ...
  const attendanceEndDate = currentDate;
  ...
  // Fetch holidays for current month (limited - holidays are usually < 20 per year)
  ...
  // Daily rate = Monthly Salary / Full Month Working Days
  // Prorated = Daily Rate × Present Days
  // This ensures: If 0.5 days present, salary = (Monthly Net / 22) × 0.5
  const workingDaysForCalculation = fullMonthWorkingDaysInfo.workingDays;
  ...
  if (staffSalary && 'basicSalary' in staffSalary && staffSalary.basicSalary) {
    ...
    // IMPORTANT: Use FULL month working days for calculation (not till current date)
    proratedSalary = calculateProratedSalary(
      calculatedSalary,
      workingDaysForCalculation, // Use FULL month working days for daily rate
    ...
    // IMPORTANT: Use FULL month working days for calculation
    proratedSalary = calculateProratedSalary(
      calculatedSalary,
      workingDaysForCalculation, // Use FULL month working days for daily rate
      presentDays // Present days till current date
  ...
  if (proratedSalary && calculatedSalary) {
    // Use FULL month working days for daily rate calculation
    const dailyNetSalary = calculatedSalary.monthly.netMonthlySalary / workingDaysForCalculation;
    const prorationFactor = presentDays / workingDaysForCalculation;
    console.log('[EmployeeDashboard] Salary Calculation:', {
      monthlyNetSalary: calculatedSalary.monthly.netMonthlySalary,
      monthlyGrossSalary: calculatedSalary.monthly.grossSalary,
      workingDaysTillCurrentDate: workingDaysInfo.workingDays,
      fullMonthWorkingDays: workingDaysForCalculation,
      presentDays,
      prorationFactor: prorationFactor.toFixed(4),
      dailyNetSalary: dailyNetSalary.toFixed(2),
      expectedProratedNet: (calculatedSalary.monthly.netMonthlySalary * prorationFactor).toFixed(2),
      proratedNetSalary: proratedSalary.proratedNetSalary.toFixed(2),
      totalFineAmount: fineInfo.totalFineAmount,
      finalNetAfterFines: proratedSalary.proratedNetSalary.toFixed(2),
      note: 'Daily rate based on FULL month working days, proration based on present days'
    });
  }
            ...
            <Card>
              ...
              <CardContent>
                {isLoadingPayroll ? (
                  ...
                  <>
                    <div className="text-2xl font-bold">{formatCurrency(stats?.currentMonthSalary ?? 0)}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {presentDays % 1 === 0 ? presentDays : presentDays.toFixed(1)} / {workingDaysInfo.workingDays}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {presentDays % 1 === 0 ? presentDays : presentDays.toFixed(1)} present out of {workingDaysInfo.workingDays} working days
                </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="p-4 border rounded-lg bg-primary/10">
                      ...
                      <div className="text-xs text-primary mt-1 font-medium">
                        {presentDays % 1 === 0 ? presentDays : presentDays.toFixed(1)} days present
                      </div>
                    </div>
                  </div>
