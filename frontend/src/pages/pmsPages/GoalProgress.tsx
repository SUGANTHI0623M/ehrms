...
import { useGetGoalsQuery, useUpdateGoalProgressMutation, Goal } from "@/store/api/pmsApi";
...
export default function GoalProgress() {
  ...
  // Fetch goals for statistics display (limited for performance)
  ...
  const handleUpdateProgress = async () => {
    ...
    if (goal && !canUpdateProgress(goal)) {
      const now = new Date();
      const goalStartDate = new Date(goal.startDate);
      const goalEndDate = new Date(goal.endDate);
      
      if (now < goalStartDate) {
        message.error(`Goal progress can only be updated from ${format(goalStartDate, "MMM dd, yyyy")} onwards.`);
      } else if (now > goalEndDate) {
        message.error(`Goal progress cannot be updated after ${format(goalEndDate, "MMM dd, yyyy")}. Please contact your manager or HR.`);
      }
      return;
    }
                ...
                <Dialog open={updateDialogOpen} onOpenChange={setUpdateDialogOpen}>
                  <DialogTrigger asChild>
                    <Button 
                      size="sm" 
                      className="gap-2"
                      disabled={!canUpdateProgress(selectedGoal)}
                      title={!canUpdateProgress(selectedGoal) 
                        ? (() => {
                            const now = new Date();
                            const goalStartDate = new Date(selectedGoal.startDate);
                            const goalEndDate = new Date(selectedGoal.endDate);
                            if (now < goalStartDate) {
