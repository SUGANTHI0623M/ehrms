...
export default function ManagerReview() {
  ...
  const queryParams = useMemo(() => {
    ...
    if (statusFilter === "all") {
      ...
      // Pending means reviews waiting for manager review - includes both self-review-submitted and manager-review-pending
      params.status = "self-review-submitted,manager-review-pending";
  ...
  // Calculate average self ratings
  const reviewsWithSelfRatings = reviewsForManager.filter((r) => r.selfReview?.overallRating);
  const avgSelfRating =
    reviewsWithSelfRatings.length > 0
      ? reviewsWithSelfRatings.reduce((sum, r) => sum + (r.selfReview?.overallRating || 0), 0) / reviewsWithSelfRatings.length
      : 0;

  // Calculate average manager ratings (from reviews where manager has submitted)
  const reviewsWithManagerRatings = reviewsForManager.filter((r) => r.managerReview?.overallRating);
  const avgManagerRating =
    reviewsWithManagerRatings.length > 0
      ? reviewsWithManagerRatings.reduce((sum, r) => sum + (r.managerReview?.overallRating || 0), 0) / reviewsWithManagerRatings.length
      : 0;
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{avgManagerRating > 0 ? avgManagerRating.toFixed(1) : "N/A"}</p>
                {avgSelfRating > 0 && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Self: {avgSelfRating.toFixed(1)}
                  </p>
                )}
              </div>
            </CardContent>
