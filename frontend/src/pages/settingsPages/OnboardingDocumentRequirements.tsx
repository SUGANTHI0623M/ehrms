...
const OnboardingDocumentRequirements: React.FC = () => {
  ...
  const [deleteRequirement, { isLoading: isDeleting }] =
    useDeleteDocumentRequirementMutation();
  const [initializeDefaults, { isLoading: isInitializingDefaults }] =
    useInitializeDefaultRequirementsMutation();
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      message.error(
        error?.data?.error?.message || "Failed to delete document requirement",
      );
    }
  };
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      message.error(
        error?.data?.error?.message || "Failed to save document requirement",
      );
    }
  };
    ...
    try {
      ...
      message.error(
        error?.data?.error?.message ||
          "Failed to initialize default requirements",
      );
    } finally {
  ...
  const columns = [
    {
      ...
      key: "order",
      width: 80,
      sorter: (a: DocumentRequirement, b: DocumentRequirement) =>
        a.order - b.order,
          ...
          title={
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-2">
              <div className="flex-1">
                ...
                <p className="text-muted-foreground text-sm sm:text-base mt-1 mb-0">
