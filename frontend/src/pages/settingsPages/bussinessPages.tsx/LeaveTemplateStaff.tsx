...
export default function LeaveTemplateStaff() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  
  const { data: templateData, isLoading: isTemplateLoading } = useGetLeaveTemplateByIdQuery(id || '', { skip: !id });
  ...
  const finalAssignedStaff = useMemo(() => {
    ...
    // Get staff from Staff model where leaveTemplateId matches (backend excludes deactivated)
    const staffFromField = allStaff.filter((staff: any) => {
      ...
      const templateId = typeof staff.leaveTemplateId === 'object' 
        ? staff.leaveTemplateId?._id 
        : staff.leaveTemplateId;
          ...
          <CardContent className="p-6">
            {finalAssignedStaff.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>No staff members assigned to this leave template.</p>
