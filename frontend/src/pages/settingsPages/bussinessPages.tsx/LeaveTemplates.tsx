...
export default function LeaveTemplates() {
  ...
  const handleCreateNew = () => {
    navigate("/business/leave-templates/new");
  };
  ...
  const handleViewStaff = (templateId: string) => {
    navigate(`/business/leave-templates/${templateId}/staff`);
  };
      ...
      <main className="p-4 space-y-6">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold">Leave Templates</h2>
              <p className="text-sm text-muted-foreground">
                Manage company leave policies and assign them to staff.
              </p>
            </div>
          </div>
          ...
          <CardContent className="p-4 space-y-4">
            {templates.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                <p>No leave templates found. Click "New Template" to create one.</p>
              </div>
            ) : (
              templates.map((template) => (
                <div
                  key={template._id}
                  className="border p-4 rounded-lg hover:bg-muted/40 transition cursor-pointer flex flex-col md:flex-row justify-between md:items-center gap-4"
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Total Leave Types: {template.leaveTypes?.length || 0}
                    </p>
