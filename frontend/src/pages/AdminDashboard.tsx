...
import TrendChart from "@/pages/lms/admin/TrendChart";
...
const AdminDashboard = () => {
  ...
  // LMS Scores & Analytics (same data as Scores & Analytics page)
  const [lmsSystemData, setLmsSystemData] = useState<any>(null);
  const [lmsSystemLoading, setLmsSystemLoading] = useState(false);
  const [chartTrendData, setChartTrendData] = useState<{
    timeSeries: { name: string; assigned: number; completed: number; avgScore: number }[];
  } | null>(null);
  useEffect(() => {
    let cancelled = false;
    setLmsSystemLoading(true);
    lmsService
      .getSystemAnalytics({ months: 12 })
      .then((res: any) => {
        if (cancelled) return;
        const payload = res?.data ?? res;
        setLmsSystemData(payload?.data ?? payload);
      })
      .catch(() => {
        if (!cancelled) setLmsSystemData(null);
      })
      .finally(() => {
        if (!cancelled) setLmsSystemLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    let cancelled = false;
    lmsService
      .getCourseAssignmentCompletionAnalytics({
        period: "monthly",
        months: CHART_TREND_MONTHS === 0 ? "0" : CHART_TREND_MONTHS,
      })
      .then((res: any) => {
        if (cancelled) return;
        const trendPayload = res?.data ?? res;
        const inner = trendPayload?.data ?? trendPayload;
        setChartTrendData(
          inner?.timeSeries != null ? { timeSeries: inner.timeSeries } : null,
        );
      })
      .catch(() => {
        if (!cancelled) setChartTrendData(null);
      });
    return () => {
      cancelled = true;
    };
  ...
  if (dashboardData?.recruitment) {
    console.log("Recruitment Data:", {
      ...
      candidateStatusDataLength: Object.entries(
        dashboardData.recruitment.candidatesByStatus || {},
      ).length,
    });
  }
  ...
  const displayStatusData = useMemo(() => {
    if (
      ...
      !Array.isArray(candidatesData.data.candidates)
    ) {
      return [];
    }
    ...
    candidates.forEach((candidate: any) => {
      const displayStatus =
        candidate.displayStatus || candidate.status || "Unknown";
  ...
  if (candidatesData?.data?.candidates) {
    console.log("Candidates Data:", {
      ...
      sampleCandidates: candidatesData.data.candidates
        .slice(0, 3)
        .map((c: any) => ({
  ...
  const goalStatusData = dashboardData?.performance?.goalsByStatus
    ? Object.entries(dashboardData.performance.goalsByStatus).map(
        ([name, value]) => ({
          name,
          value,
        }),
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Comprehensive analytics across all HRMS modules
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.recruitment?.totalJobOpenings || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.recruitment?.totalCandidates || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.recruitment?.avgTimeToHire || 0} days
                </div>
              </CardContent>
            </Card>
          </div>
                ...
                <div className="space-y-3">
                  ...
                  dashboardData.recruitment.upcomingInterviews.length > 0 ? (
                    dashboardData.recruitment.upcomingInterviews
                      ...
                      .map((interview, idx) => (
                        <div
                          key={`interview-${idx}-${interview.name || idx}`}
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Candidates with "Interview" status will appear here
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.staff?.totalEmployees || 0}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Active: {dashboardData?.staff?.activeEmployees || 0} |
                  Inactive: {dashboardData?.staff?.inactiveEmployees || 0}
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.staff?.recentOnboardings || 0}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Last 30 days
                </p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.staff?.attritionCount || 0}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatINR(dashboardData?.staff?.salaryOverview?.averageGross)}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Total: {formatINR(dashboardData?.staff?.salaryOverview?.totalGross)}
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                {departmentData.length > 0 ? (
                  <ChartContainer config={{}} className="h-[300px] w-[100%]">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.performance?.totalGoals || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.performance?.goalProgress?.avgProgress
                    ? dashboardData.performance.goalProgress.avgProgress.toFixed(
                        1,
                      )
                    : 0}
                  %
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.performance?.complianceStatus?.complianceRate
                    ? parseFloat(
                        dashboardData.performance.complianceStatus
                          .complianceRate,
                      ).toFixed(1)
                    : 0}
                  %
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <Badge>
                      {dashboardData?.performance?.selfReviewsSubmitted || 0}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <span className="font-medium">Manager Reviews Pending</span>
                    <Badge variant="destructive">
                      {dashboardData?.performance?.managerReviewsPending || 0}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <span className="font-medium">HR Reviews Pending</span>
                    <Badge variant="destructive">
                      {dashboardData?.performance?.hrReviewsPending || 0}
                    </Badge>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card
                ...
                onClick={() =>
                  (window.location.href = "/staff/leaves-pending-approval")
                }
              >
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                  <CardTitle className="text-sm font-medium">
              ...
              <Card
                className="cursor-pointer hover:shadow-md transition-shadow"
                onClick={() => (window.location.href = "/staff/loans")}
              >
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                  <CardTitle className="text-sm font-medium">
                    Pending Loans
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium">
                  Payroll Cycles Completed
                </CardTitle>
                <FileText className="w-4 h-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.payroll?.payrollCyclesCompleted || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.payroll?.payrollPending || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatINR(dashboardData?.payroll?.totalPayrollProcessed)}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.payroll?.reimbursements?.pending || 0}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Approved:{" "}
                  {dashboardData?.payroll?.reimbursements?.approved || 0}
                </p>
              </CardContent>
            </Card>
          </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <Badge variant="default">
                      {dashboardData?.payroll?.attendanceSummary?.present || 0}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <span className="font-medium">Absent</span>
                    <Badge variant="destructive">
                      {dashboardData?.payroll?.attendanceSummary?.absent || 0}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <span className="font-medium">Late</span>
                    <Badge variant="outline">
                      {dashboardData?.payroll?.attendanceSummary?.late || 0}
                    </Badge>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <Badge>
                      {dashboardData?.payroll?.reimbursements?.submitted || 0}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 border rounded-lg">
        ...
        <div className="space-y-4">
          ...
          {lmsSystemLoading ? (
            ...
            <>
              {/* All KPI cards (from Scores & Analytics) */}
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 min-[991px]:grid-cols-3 min-[1201px]:grid-cols-6 gap-4">
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <CardTitle className="text-sm font-medium">
                      Total Learners
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {lmsSystemData?.kpi?.totalEnrollments ?? 0}
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {lmsSystemData?.kpi?.completionRate ?? 0}%
                    </div>
                  </CardContent>
                </Card>
                <Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {lmsSystemData?.kpi?.activeLearnerCount ?? 0}
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {lmsSystemData?.kpi?.avgTimeToCompleteDays ?? 0} days
                    </div>
                  </CardContent>
                </Card>
              </div>
              {/* Trend (Assigned % vs Completed % vs Avg Score) + Score Distribution */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <Card className="lg:col-span-2 overflow-visible">
                  <CardContent className="pt-6 min-h-[380px]">
                    <div className="w-full min-w-0">
                      {!chartTrendData?.timeSeries?.length ? (
                        <div className="flex items-center justify-center min-h-[320px] text-muted-foreground text-sm">
                          No trend data for selected period
                        </div>
                      ) : (
                        <TrendChart
                          title="Trend (Assigned % vs Completed % vs Avg Score)"
                          trendMonths={chartTrendData.timeSeries.map(
                            (m: {
                              name: string;
                              assigned: number;
                              completed: number;
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="h-[320px]">
                      {!lmsSystemData?.scoreDist?.length ||
                      lmsSystemData?.scoreDist?.every((d: any) => !d.value) ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <RechartsPieChart>
                            <Pie
                              data={(lmsSystemData?.scoreDist ?? []).filter(
                                (d: any) => d.value > 0,
                              )}
                              innerRadius={50}
                              outerRadius={80}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.assets?.totalAssetTypes || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Allocated: {dashboardData?.assets?.allocatedAssets || 0} |
                  Unallocated: {dashboardData?.assets?.unallocatedAssets || 0}
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.assets?.assetsByStatus?.Working || 0}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {dashboardData?.assets?.assetsByStatus?.[
                    "Under Maintenance"
                  ] || 0}
                </div>
              </CardContent>
                ...
                <>
                  <ChartContainer config={{}} className="h-[300px] w-[100%]">
                    <RadialBarChart
                      ...
                      barSize={Math.max(
                        10,
                        Math.min(30, 300 / assetStatusData.length),
                      )}
                      data={radialBarData}
                      startAngle={90}
                      endAngle={-270}
