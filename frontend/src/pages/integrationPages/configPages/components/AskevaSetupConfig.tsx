...
const AskevaSetupConfig = ({
  ...
  const { data: credentialsData, refetch: refetchCredentials } =
    useGetAskevaCredentialsQuery(undefined, {
  ...
  const [syncTemplates, { isLoading: isSyncing }] =
    useSyncAskevaTemplatesMutation();
  ...
  const { data: allTemplatesData } = useGetAskevaTemplatesQuery({
    page: 1,
    limit: 50, // Reduced from 1000 - enough for dropdown
  });
  const [mapTemplate] = useMapAskevaTemplateMutation();
  const { data: webhookLogsData } = useGetAskevaWebhookLogsQuery({
    limit: 50,
    offset: 0,
  });
  const { data: eventMappingsData, refetch: refetchEventMappings } =
    useGetEventTemplateMappingsQuery();
  const [saveEventMapping, { isLoading: isSavingMapping }] =
    useSaveEventTemplateMappingMutation();
  const [deleteEventMapping, { isLoading: isDeletingMapping }] =
    useDeleteEventTemplateMappingMutation();
  const [editingMappingId, setEditingMappingId] = useState<string | null>(null);
  const [isMappingModalVisible, setIsMappingModalVisible] = useState(false);
  const [selectedEventForMapping, setSelectedEventForMapping] =
    useState<any>(null);
  ...
  useEffect(() => {
    if (isEditing && configData?.data?.config && credentialsData?.data) {
      setBackendUrl(
        credentialsData.data.backendUrl || "https://backend.askeva.io",
      );
              ...
              <FormField
                ...
                tooltip={
                  isConfigured && !isEditing
                    ...
                    : "Enter your API token from AskEVA"
                }
              >
                {isConfigured && !isEditing ? (
                  <Input
  ...
  const templatesTabContent = useMemo(() => {
    ...
    const handleMapTemplate = async (
      ...
      try {
        ...
        message.error(
          err.data?.error?.message || "Failed to update template mapping",
        );
      }
    };
          ...
          return (
            <Space wrap size={[4, 4]}>
              {mappedEvents.map((event) => {
                ...
                return (
                  <Tag key={event} color="green" style={{ margin: 0 }}>
                    {eventLabel}
                  </Tag>
                );
              })}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <Switch
                ...
                onChange={async (checked) => {
                  try {
                    message.info(
                      "Template status toggle - feature coming soon",
                    );
                  } catch (err) {
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <Text type="secondary">
              Manage and sync message templates from ASKEVA
            </Text>
          </div>
          <Space>
            <Button
        ...
        {config?.lastSyncedAt && (
          <Alert
            ...
            description={
              config.templateSyncStatus === "success"
  ...
  const eventMappingTabContent = useMemo(() => {
    ...
    const allTemplatesForDropdown =
      allTemplatesData?.data?.templates || templates;
    const approvedTemplates = allTemplatesForDropdown.filter(
      (t: any) => t.status === "APPROVED" && t.isActive !== false,
    );
    ...
    const eventTypes = [
      ...
      },
      {
        ...
        description: "Reminder sent before interview",
      },
      {
        value: "offer_sent",
        label: "Offer Sent",
        description: "Triggered when an offer letter is sent",
      },
      {
        value: "offer_accepted",
        label: "Offer Accepted",
        description: "Triggered when candidate accepts offer",
      },
      {
        value: "onboarding_started",
        label: "Onboarding Started",
        description: "Triggered when onboarding begins",
      },
      {
        value: "onboarding_completed",
        label: "Onboarding Completed",
        description: "Triggered when onboarding is completed",
      },
      {
      ...
      {
        value: "referral_link_shared",
        label: "Referral Link Shared",
        description: "Triggered when a referral link is shared via WhatsApp or Email",
      },
    ];
    ...
    const hrmsFields = [
      ...
      { value: "referralLink", label: "Referral Link", category: "Referral" },
      { value: "referralLinkUrl", label: "Referral Link URL", category: "Referral" },
    ];
    ...
    const handleEditMapping = (mapping: any) => {
      ...
      const template = templates.find(
        (t: any) =>
          t._id === mapping.templateId?._id || t._id === mapping.templateId,
      );
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            ...
            message.error(
              err.data?.error?.message || "Failed to delete mapping",
            );
          }
        },
      });
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (_: any, record: any) => {
          const template =
            typeof record.templateId === "object"
              ? record.templateId
      ...
      <div style={{ padding: "0 8px" }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <Text type="secondary">
              Configure templates and variable mappings for each HRMS event
            </Text>
          </div>
          <Button
            type="primary"
              ...
              locale={{
                emptyText: (
                  <div style={{ padding: "40px 0", textAlign: "center" }}>
                    <Text type="secondary">
                      No event template mappings found. Click "Create New
          ...
          <EventMappingModal
            ...
            onSave={async (mappingData: any) => {
              try {
                ...
                message.error(
                  err.data?.error?.message || "Failed to save mapping",
                );
              }
            }}
            onCancel={() => {
  ...
  const tabItems = useMemo(
    ...
    [
      ...
      templatesTabContent,
      eventMappingTabContent,
      webhookLogsTabContent,
    ],
  );
          ...
          <Space direction="vertical" size={8} style={{ width: "100%" }}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              >
                ...
                />
                <span>ASKEVA WhatsApp Integration</span>
              </Title>
              <Space wrap size={[8, 8]}>
                <Tag
...
const EventMappingModal = ({
  ...
  const [selectedEventType, setSelectedEventType] = useState<string>(
    event?.value || mapping?.hrmsEventType || "",
  );
  const [selectedTemplateId, setSelectedTemplateId] = useState<string>(
    template?._id || mapping?.templateId?._id || mapping?.templateId || "",
  );
  ...
  useEffect(() => {
    if (selectedTemplateId && visible) {
      const selectedTemplate = templates.find(
        (t: any) => t._id === selectedTemplateId,
      );
      if (selectedTemplate) {
        const variables = extractVariables(selectedTemplate.components || []);
        const templateChanged = prevTemplateIdRef.current !== selectedTemplateId;
        prevTemplateIdRef.current = selectedTemplateId;

        // Only update mappings if template actually changed or if we're creating new mapping
        if (templateChanged || (!editingId && variableMappings.length === 0)) {
          // If editing and has existing mappings, preserve them but add any new variables
          if (editingId && variableMappings.length > 0 && templateChanged) {
            const existingVars = new Set(
              variableMappings.map((m: any) => m.templateVariable),
            );
            const newVars = variables.filter((v: string) => !existingVars.has(v));
            const currentVars = new Set(variables);

            // Update mappings: keep existing ones that are still in template, add new ones, remove old ones
            const updatedMappings = variableMappings
              .filter((m: any) => currentVars.has(m.templateVariable))
              .concat(
                newVars.map((varName) => ({
  ...
  }, [selectedTemplateId, templates, extractVariables, visible, editingId]);
  ...
  useEffect(() => {
    if (visible) {
      ...
      const timer = setTimeout(() => {
        if (mapping) {
          const templateId = mapping.templateId?._id || mapping.templateId;
          form.setFieldsValue({
  ...
  const handleSave = async () => {
    try {
      ...
      const invalidMappings = variableMappings.filter(
        (m) => !m.hrmsField && !m.defaultValue,
      );
      if (invalidMappings.length > 0) {
        const missingVars = invalidMappings
          .map((m) => m.templateVariable)
          .join(", ");
        message.error(
          `Please map all template variables to HRMS fields or provide default values. Missing: ${missingVars}`,
        );
        return;
      }

      // Ensure all variables from template are included
      const templateVars = extractVariables(
        selectedTemplateObj.components || [],
      );
      
      // Update selectedTemplateId state to match form value
      ...
      const missingVars = templateVars.filter(
        (v: string) => !mappedVars.has(v),
      );

      if (missingVars.length > 0) {
        message.error(
          `Missing mappings for template variables: ${missingVars.join(", ")}`,
        );
        return;
      }
  ...
  return (
    <Modal
      title={
        editingId
          ? "Edit Event Template Mapping"
          : "Create Event Template Mapping"
      }
      open={visible}
      onOk={handleSave}
      onCancel={() => {
    ...
    >
      <Form 
        ...
        onValuesChange={(changedValues, allValues) => {
          // Sync state when form values change
          ...
          if (changedValues.hrmsEventType !== undefined) {
            setSelectedEventType(changedValues.hrmsEventType || "");
          }
        }}
        key={editingId || 'new'}
      >
        ...
        <Form.Item label="Status" name="isEnabled" valuePropName="checked">
            ...
            {variableMappings.length === 0 ? (
              ...
              <>
                <Alert
                  ...
                  description={
                    <div>
                      <Text strong>Required:</Text> All template variables must
                      be mapped to HRMS fields or have default values.
                      <br />
                  ...
                  type={
                    variableMappings.every(
                      (m: any) => m.hrmsField || m.defaultValue,
                    )
                      ? "success"
                      : "warning"
                  }
                ...
                {variableMappings.map((mapping, index) => {
                  const isMapped = !!(
                    mapping.hrmsField || mapping.defaultValue
                  );
                      ...
                      <Row gutter={[16, 8]}>
                        ...
                        <Col xs={24} sm={10}>
                          <Form.Item
                            ...
                            help={
                              !mapping.hrmsField && !mapping.defaultValue
                                ? "Required: Map to HRMS field or provide default value"
                                : ""
                            }
                            style={{ marginBottom: 0 }}
                          >
                            <Select
                              ...
                              value={mapping.hrmsField || undefined}
                              onChange={(value) => {
                        ...
                        <Col xs={24} sm={6}>
                          <Form.Item
                            ...
                            help={
                              !mapping.hrmsField && !mapping.defaultValue
                                ? "Required if no HRMS field"
                                : "Use if HRMS field value is missing"
                            }
                            style={{ marginBottom: 0 }}
                          >
                            <Input
                              placeholder={
                                !mapping.hrmsField ? "Required" : "Optional"
                              }
