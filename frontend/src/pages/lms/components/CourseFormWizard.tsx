...
import { ArchiveIcon } from "@/components/lms/SharedComponents";
...
function SwitchWithLabel({
  ...
  return (
    <div className="flex items-center gap-3">
      <Switch checkedChildren="YES" unCheckedChildren="NO" checked={checked} onChange={handleChange} />
      <Text type="secondary" className="text-sm">{label}</Text>
    </div>
  );
}

const CourseFormWizard = ({
  open,
  onClose,
  onSuccess,
  initialData,
  ...
  const savedMaterialFilesRef = useRef<
    Map<string, { mIndex: number; file: File | Blob; fileName: string }[]>
  >(new Map());
  ...
  const pendingMaterialFilesRef = useRef<
    Map<number, { file: File | Blob; fileName: string }>
  >(new Map());
  ...
  /** When editing, load lessons from initialData only once when dialog opens so we don't overwrite in-memory lessons (e.g. newly uploaded material URLs) when parent re-renders. */
  const initialLessonsLoadedRef = useRef(false);
  /** Ref for curriculum step container; used to prevent native form submit (page refresh) when selecting files. */
  ...
  /**
   * CREATE vs EDIT behavior (kept strictly separate):
   *
   * CREATE (new course):
   * - No backend course exists until user clicks "Save Draft" or "Publish Course".
   * - Step 2: file selection only stores files in state/refs. No upload to storage. No upload progress.
   * - If user cancels: nothing is created, no orphan files in storage.
   * - Upload and progress run only on final submit (Publish/Save Draft).
   *
   * EDIT (existing course):
   * - Course already exists. Only the lesson being saved is updated in state.
   * - Step 2: file selection stores files in state/refs (same as create). Upload runs only when
   *   user clicks "Save Lesson Group" for that lesson (no immediate upload on select → no orphans).
   * - "Save Lesson Group": uploads new/replaced files for that lesson only, then updates only that
   *   lesson in state (replace by id). New lesson = append with new id. No duplicate lessons.
   * - Final submit: full course payload with lessons deduped by id; existing material URLs preserved.
   */

  // --- Effects ---
  // Depend on course id only so parent re-renders with new initialData ref (e.g. after window focus refetch) don't re-run and reset step/lessons
  const initialDataId = initialData?._id ?? initialData?.id ?? null;
  useEffect(() => {
    if (!open) {
      initialLessonsLoadedRef.current = false;
      return;
    }
    fetchOrgData();
    if (initialData) {
      // Populate form and lessons only on first load for this dialog open so we don't overwrite
      // in-memory lessons (e.g. newly uploaded material URLs) when parent re-renders with new initialData ref
      const shouldLoadLessons = !initialLessonsLoadedRef.current;
      if (shouldLoadLessons) initialLessonsLoadedRef.current = true;

      // Coerce toggles from initialData (must match saved course so edit shows correct state)
      const duration = initialData.completionDuration || {};
      const isMandatory = initialData.isMandatory === true || initialData.isMandatory === "true" || !!initialData.isMandatory;
      const isLiveAssessment = initialData.isLiveAssessment === true || initialData.isLiveAssessment === "true" || !!initialData.isLiveAssessment;
      setAssessmentMode(isLiveAssessment);

      // Set all Details fields; do NOT resetFields() — it resets to Form initialValues (false) and can leave toggles OFF
      form.setFieldsValue({
      ...
      // Thumbnail and lessons only on first load so we don't overwrite in-memory state when parent re-renders
      if (shouldLoadLessons) {
        setThumbnailPreview(getFileUrl(initialData.thumbnailUrl) || "");
      }

      // Populate Lessons only on first load for this dialog open (avoid overwriting in-memory edits e.g. uploaded material URLs)
      if (shouldLoadLessons) {
        let loadedLessons: Lesson[] = [];

                    if (initialData.lessons && Array.isArray(initialData.lessons) && initialData.lessons.length > 0) {
                        // 1. New Structured Logic (Prefer this) — use string IDs so edit/save matching works
                        const toStr = (v: any) => (v != null ? String(v) : '');
                        loadedLessons = initialData.lessons.map((l: any) => ({
                            id: toStr(l._id) || toStr(l.id) || Math.random().toString(36).substr(2, 9),
                            title: l.title ?? '',
                            materials: (l.materials || []).map((m: any) => {
                                let typeLabel: LessonMaterial['contentType'] = 'External URL';
                                const mt = (m.type && String(m.type).toUpperCase()) || '';
                                if (mt === 'VIDEO') typeLabel = 'Video';
                                else if (mt === 'YOUTUBE') typeLabel = 'YouTube';
                                else if (mt === 'PDF') typeLabel = 'PDF';
                                else if (mt === 'URL' || (m.content && mt !== 'VIDEO' && mt !== 'YOUTUBE' && mt !== 'PDF')) typeLabel = 'External URL';

                                const materialUrl = m.url ?? m.filePath ?? m.link ?? '';
                                return {
                                    id: toStr(m._id) || toStr(m.id) || Math.random().toString(36).substr(2, 9),
                                    title: m.title ?? '',
                                    contentType: typeLabel,
                                    url: typeof materialUrl === 'string' ? materialUrl : String(materialUrl || ''),
                                    subtitleUrl: m.subtitleUrl,
                                    publicId: m.publicId,
                                    format: m.format,
                                    provider: (materialUrl && String(materialUrl).includes('cloudinary.com')) ? 'cloudinary' : (m.provider || 'local')
                                };
                            }),
                            questions: (l.questions || []).map((q: any) => ({ ...q }))
                        }));
                    } else {
                        ...
                        (initialData.materials || []).forEach((m: any) => {
                            ...
                            else if (mt === 'URL' || m.content) typeLabel = 'External URL';

                            if (!groupedMap[lTitle]) groupedMap[lTitle] = [];
                            const materialUrl = m.url ?? m.filePath ?? '';
                ...
                // Reset only when opening for create (no initialData). Do not reset when initialData is missing but might be set next (edit flow).
                const isCreateIntent = !initialData || !initialData._id;
                if (isCreateIntent) {
                    form.resetFields();
    ...
    // Set toggle values before paint when opening edit so Details tab shows correct state immediately
    useLayoutEffect(() => {
        if (!open || !initialData) return;
        const isMandatory = initialData.isMandatory === true || initialData.isMandatory === "true" || !!initialData.isMandatory;
        const isLiveAssessment = initialData.isLiveAssessment === true || initialData.isLiveAssessment === "true" || !!initialData.isLiveAssessment;
        form.setFieldsValue({ isMandatory, isLiveAssessment });
        setAssessmentMode(isLiveAssessment);
    }, [open, initialData?._id, initialData?.id, initialData?.isMandatory, initialData?.isLiveAssessment]);

    // Fallback: sync toggles after paint so we win over Form's initialValues (multiple delays to beat any race)
    useEffect(() => {
        if (!open || !initialData) return;
        const isMandatory = initialData.isMandatory === true || initialData.isMandatory === "true" || !!initialData.isMandatory;
        const isLiveAssessment = initialData.isLiveAssessment === true || initialData.isLiveAssessment === "true" || !!initialData.isLiveAssessment;
    ...
    // Reset load ref on unmount so remount (e.g. Strict Mode or navigate back) loads initial data again
    useEffect(() => {
        return () => {
            initialLessonsLoadedRef.current = false;
        };
    ...
    // Prevent native form submit while modal is open so page never refreshes (any step: Details, Curriculum, Review)
    useEffect(() => {
        if (!open) return;
  ...
  const fetchOrgData = async () => {
    ...
    const errors: string[] = [];
    try {
      const [deptResult, empResult, catResult, langResult] =
      ...
      if (deptResult.status === "fulfilled") {
        ...
        const msg =
          (deptResult.reason?.response?.data?.error?.message ??
            deptResult.reason?.response?.data?.message ??
      ...
      if (empResult.status === "fulfilled") {
        ...
        const msg =
          (empResult.reason?.response?.data?.error?.message ??
            empResult.reason?.response?.data?.message ??
            empResult.reason?.message) ||
          "Employees";
      ...
      if (catResult.status === "rejected") {
        const msg =
          (catResult.reason?.response?.data?.error?.message ??
            catResult.reason?.response?.data?.message ??
      ...
      if (langResult.status === "rejected") {
        const msg =
          (langResult.reason?.response?.data?.error?.message ??
            langResult.reason?.response?.data?.message ??
  ...
  /** Programmatic material upload (e.g. for future use). UI no longer uploads on file select; EDIT defers to Save Lesson Group, CREATE to Publish/Save Draft. */
  const uploadingMaterialRef = useRef<Set<number>>(new Set());
  const uploadMaterialImmediately = async (
    name: number,
    file: File | Blob,
    type: "Video" | "PDF",
  ) => {
    if (!isEditMode) return; // CREATE: never upload until final submit
    const materialId =
      inlineLessonForm.getFieldValue(["materials", name, "id"]) ||
      `temp-${name}`;
    ...
    try {
      const res = await lmsService.uploadMaterial(
      ...
      if (res?.success && res?.data) {
        ...
        message.success(
          `${type === "Video" ? "Video" : "PDF"} uploaded — you can edit the title below`,
        );
      } else {
        const errMsg =
  ...
  /** Subtitle upload: in edit mode only, still uploads on drop so URL is set before Save Lesson Group. CREATE defers to Publish/Save as Draft. */
  const uploadingSubtitleRef = useRef<Set<number>>(new Set());
  const uploadSubtitleImmediately = async (name: number, file: File) => {
    if (!isEditMode) return; // CREATE: defer until Publish/Save as Draft
    const materialId =
      inlineLessonForm.getFieldValue(["materials", name, "id"]) ||
      `temp-${name}`;
  ...
  const startEditLesson = (lesson: Lesson) => {
    ...
    // Restore all material fields so "Uploaded" shows for existing files; url can come from url or filePath (API shape)
    const materialsWithFilePlaceholder = lesson.materials.map((m) => {
      const existingUrl = (m as any).url ?? (m as any).filePath ?? "";
      return {
        ...m,
        id: m.id || Math.random().toString(36).substr(2, 9),
        title: m.title ?? "",
        ...
        file: [], // no File in state; restored from ref below if we have pending files for this lesson
        fileName:
          (m as any).fileName ??
          (existingUrl
            ? m.title ||
    ...
    inlineLessonForm.setFieldsValue({
      ...
      questions: (lesson.questions || []).map((q) => {
        if (q.type === "MCQ" || q.type === "Multiple Correct") {
          const indices = q.correctAnswers
            .map((ans) => {
    ...
    if (refFiles && refFiles.length > 0) {
      const currentMaterials =
        inlineLessonForm.getFieldValue("materials") || [];
      const updated = currentMaterials.map((m: any, i: number) => {
        const refEntry = refFiles.find((e) => e.mIndex === i);
        if (
          refEntry &&
          (refEntry.file instanceof File || refEntry.file instanceof Blob)
        ) {
          pendingMaterialFilesRef.current.set(i, {
            file: refEntry.file,
            fileName: refEntry.fileName,
          });
          return {
            ...m,
            file: [{ originFileObj: refEntry.file, name: refEntry.fileName }],
          };
        }
        return m;
      });
  ...
  const cancelLessonEdit = () => {
    ...
    if (lessonIdToRestore && snapshot) {
      setLessons((prev) =>
        prev.map((l) => (l.id === lessonIdToRestore ? snapshot : l)),
      );
    }
  };
  ...
  const saveInlineLesson = async () => {
    try {
      ...
      const materialsArray = Array.isArray(values.materials)
        ? values.materials
        : [];
      ...
      let processedMaterials = materialsArray.map(
        (m: any, matIndex: number) => {
          const isFileMaterial =
            m.contentType === "Video" || m.contentType === "PDF";
          let uploadedFile: File | Blob | null = null;
          let fileNameFromPending: string | undefined;
          if (isFileMaterial) {
            const formFile = inlineLessonForm.getFieldValue([
              "materials",
              matIndex,
              "file",
            ]);
          ...
          return {
            ...
            fileName:
              fileNameFromPending ||
              (uploadedFile instanceof File ? uploadedFile.name : undefined) ||
              m.fileName,
            publicId: m.publicId,
            format: m.format,
            provider: m.provider,
      ...
      // CREATE: never upload here; files stay in state/refs until Publish/Save as Draft (no progress modal in Step 2).
      // EDIT: upload new/replaced files only when user clicks Save Lesson Group (progress modal shown here).
      ...
      if (isEditMode && toUpload.length > 0) {
        ...
        try {
          const results = await Promise.all(
            toUpload.map(async ({ m, matIndex }) => {
              try {
                ...
                let fileToSend: File | Blob | null =
                  m.file instanceof File || m.file instanceof Blob
                    ? m.file
                    : null;
                if (!fileToSend) {
                  fileToSend =
                    pendingMaterialFilesRef.current.get(matIndex)?.file ?? null;
                }
                if (!fileToSend) {
                  const formVal = inlineLessonForm.getFieldValue([
                    "materials",
                    matIndex,
                    "file",
                  ]);
          ...
          message.error(
            "Some file uploads failed. You can still save and try again on final submit.",
          );
        }
      }
      ...
      const processedQuestions = (values.questions || []).map((q: any) => {
        let answers: string[] = [];
        if (q.type === "MCQ" || q.type === "Multiple Correct") {
          const raw = Array.isArray(q.correctAnswers)
            ? q.correctAnswers
            : [q.correctAnswers];
          answers = raw
            .map((idx: string) => q.options[parseInt(idx)])
            .filter(Boolean);
      ...
      // When Live Assessment is off: at least 1 question per lesson when saving the lesson (step 2)
      if (!form.getFieldValue("isLiveAssessment")) {
        if (!processedQuestions || processedQuestions.length === 0) {
          const lessonTitle = values.title?.trim() || "Untitled lesson";
          message.error(
            `When Live Assessment is off, every lesson must have at least one assessment question. Add questions to: ${lessonTitle}`,
          );
          return;
        }
      }
      ...
      const lessonId =
        editingLessonId || Math.random().toString(36).substr(2, 9);
        ...
        .map((m, mIndex) => {
          ...
          if (f instanceof File || f instanceof Blob) {
            return {
              mIndex,
              file: f,
              fileName: (m.fileName as string) || `material-${mIndex}`,
            };
          }
          return null;
        })
      ...
      // EDIT: update only this lesson (replace by id). CREATE: append new lesson (new id). No duplicates.
      if (editingLessonId) {
        setLessons((prev) =>
      ...
      const errMsg =
        ...
        (typeof error === "string"
          ? error
          : "Please complete all required fields (Lesson Title, Material type, and file/URL for each material).");
      message.error(errMsg);
    }
  };
  ...
  const handleNext = async () => {
    try {
      if (currentStep === 0) {
        ...
        if (isAddingLesson) {
          message.warning(
            "Please save or cancel the current lesson edit first.",
          );
          return;
        }
        if (lessons.length === 0) {
  ...
  const onFinalSubmit = async (status: "Draft" | "Published" | "Archived") => {
    ...
    const actionLabel =
      status === "Draft"
        ...
        : status === "Published"
          ? "publish"
          : "archive";
    const saveMsg = message.loading(
      `Preparing to ${actionLabel} course...`,
      0,
    );
    ...
    try {
      ...
      // 1. Structure Lessons with Materials and Questions (dedupe by id to prevent duplicate lessons)
      const dedupedLessons = Array.from(
        new Map(lessons.map((l) => [l.id, l])).values()
      );
      let structuredLessons = dedupedLessons.map((lesson, lIndex) => {
        const materials = lesson.materials.map((material, mIndex) => {
          let fileToUpload = extractRawFile(material.file);
          if (
            !fileToUpload &&
            (material.contentType === "Video" || material.contentType === "PDF")
          ) {
            const fromRef = savedMaterialFilesRef.current
              .get(lesson.id)
              ?.find((e) => e.mIndex === mIndex);
            if (fromRef) {
              fileToUpload = fromRef.file;
              if (!material.fileName)
                (material as any).fileName = fromRef.fileName;
            }
          }
      ...
      // When Live Assessment is off: at least 1 question per lesson is required
      if (!formData.isLiveAssessment) {
        const lessonsWithoutQuestions = structuredLessons
        ...
        if (lessonsWithoutQuestions.length > 0) {
          const names = lessonsWithoutQuestions
            .map(({ lesson }) => lesson.title || "Untitled lesson")
            .join(", ");
          message.error(
            `When Live Assessment is off, every lesson must have at least one assessment question. Add questions to: ${names}`,
          );
      ...
      if (!isEditMode && materialFilesToAppend.length > 0) {
        ...
        try {
            ...
            materialFilesToAppend.map(async (item, idx) => {
              try {
                ...
                const fileForUpload = toFile(
                  item.file,
                  item.fileName ||
                    (apiType === "VIDEO" ? "video.mp4" : "document.pdf"),
                );
                const res = await lmsService.uploadMaterial(
                  fileForUpload,
                  apiType,
          ...
          if (failed.length > 0) {
            ...
            message.error(
              `${failed.length} file(s) failed to upload. Please try again or remove those materials.`,
            );
            setSubmittingStatus(null);
            saveMsg();
            return;
          ...
          structuredLessons = structuredLessons.map((lesson, lIdx) => ({
            ...
            materials: lesson.materials.map((mat, mIdx) => {
              const result = uploadResults.find(
                (r) =>
                  r.lessonIndex === lIdx && r.materialIndex === mIdx && r.url,
              );
              if (result?.url) {
                return {
                  ...mat,
      ...
      // 3. Construct the Course Metadata (assignment is never sent — managed only via Assign Learners)
      ...
      // Ensure assignment is never sent from wizard (edit must not change who the course is assigned to)
      delete payload.assignmentType;
      delete payload.departments;
      delete payload.assignedEmployees;
      ...
      const uploadLabel =
        status === "Published"
          ...
          : status === "Archived"
            ? "Archiving"
            : "Saving";
      const actionMsg = message.loading(
        `${uploadLabel} course and uploading media... This may take a moment for large files.`,
        0,
      );
      ...
      if (initialData && initialData._id) {
        ...
        actionMsg();
        message.success(
          `Course ${status === "Draft" ? "saved as draft" : "published"}!`,
        );
      }
      ...
      const errMsg =
        ...
        errData?.message ??
        error.message ??
        "Failed to save course. Please check all fields.";
      message.error(errMsg);
    } finally {
      setSubmittingStatus(null);
    }
  ...
  const handleMaterialPreview = (name: number) => {
    ...
    const rawFile =
      extractRawFile(material?.file) ||
      pendingMaterialFilesRef.current.get(name)?.file;
    const hasUrl = !!rawUrl;
    const hasFile = !!(
      rawFile &&
      (rawFile instanceof File || rawFile instanceof Blob)
    );
    ...
    setMaterialPreview({
      ...
      ...(type === "video" && material.subtitleUrl
        ? {
            subtitleUrl:
              getFileUrl(material.subtitleUrl) || material.subtitleUrl,
          }
        : {}),
    });
  };
    ...
    >
      <div className="w-full h-full bg-black flex items-center justify-center overflow-hidden rounded-b-lg">
        {materialPreview.type === "video" && (
          ...
          >
            {materialPreview.subtitleUrl && (
              <track
                kind="subtitles"
                src={materialPreview.subtitleUrl}
                default
              />
            )}
          </video>
    ...
    <Row gutter={[24, 24]}>
      ...
      <Col xs={24} lg={15}>
        <Card bordered={false} className="shadow-sm mb-6">
          <Form.Item
            ...
            rules={[{ required: true, min: 5, message: "Min 5 chars allowed" }]}
          >
                  ...
                  <div className="flex justify-between items-center w-full group">
                    ...
                    {c.id && (
                      <Button
                        ...
                        onClick={(e) =>
                          handleDeleteMetaOption(e, "CATEGORY", c.id!, c.value)
                        }
                      />
                    )}
                  </div>
              ...
              onInputKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  if (
                    val.length >= 2 &&
                    !languages.some(
                      (l) => l.value.toLowerCase() === val.toLowerCase(),
                    )
                  ) {
                    e.preventDefault();
                    e.stopPropagation();
                  ...
                  <div className="flex justify-between items-center w-full group">
                    ...
                    {l.id && (
                      <Button
                        ...
                        icon={<CloseOutlined style={{ fontSize: "12px" }} />}
                        onClick={(e) =>
                          handleDeleteMetaOption(e, "LANGUAGE", l.id!, l.value)
                        }
                      />
                    )}
                  </div>
          ...
          <Form.Item
            ...
            rules={[
              { required: true, min: 20, message: "Min 20 chars allowed" },
            ]}
          >
            <TextArea
              rows={5}
              showCount
              maxLength={1000}
              placeholder="Course objective and summary..."
            />
          </Form.Item>
      ...
      <Col xs={24} lg={9}>
        <div className="">
          ...
          <div
            className="relative w-full rounded-lg overflow-hidden border border-gray-200 bg-gray-50 mb-6 group"
          ...
          >
            {thumbnailPreview ? (
              ...
              <div
                className="flex flex-col items-center justify-center h-full text-gray-400 cursor-pointer hover:bg-gray-100 transition-colors"
          ...
          >
            {/* Form.Item injects checked/onChange into its direct child. Wrapper must forward them to Switch so the toggle reflects form value. */}
            <SwitchWithLabel label="Is this a Mandatory Course?" />
          </Form.Item>
          ...
          <Form.Item
            ...
            label={
              <Space>
                ...
                <Tooltip title="If ON, assessment is conducted live. If OFF, it's Standardized Assessment — every lesson must have at least one assessment question.">
                  <InfoCircleOutlined className="text-emerald-500" />
                </Tooltip>
              </Space>
            }
  ...
  const renderStep2 = () => {
    const lessonFormBody = (
      <>
                ...
                >
                  <Input
                    placeholder="e.g. Module 1: Introduction to Safety"
                    size="large"
                  />
                </Form.Item>
                  ...
                  {(fields, { add, remove }) => (
                    <div className="space-y-6">
                      {fields.map(({ key, name, ...restField }) => {
                        const type = inlineLessonForm.getFieldValue([
                          "materials",
                          name,
                          "contentType",
                        ]);
                        const hasFile =
                          !!extractRawFile(
                            inlineLessonForm.getFieldValue([
                              "materials",
                              name,
                              "file",
                            ]),
                          ) || !!pendingMaterialFilesRef.current.get(name);
                        const hasUrl = !!inlineLessonForm
                          .getFieldValue(["materials", name, "url"])
                          ?.trim?.();
                        const hasContent = !!inlineLessonForm
                          .getFieldValue(["materials", name, "content"])
                          ?.trim?.();
                        const hasInput =
                          type === "Video" || type === "PDF"
                            ? hasFile
                            : type === "YouTube" || type === "External URL"
                              ? hasUrl
                              : type === "Text/Article"
                                ? hasContent
                                : false;
                        ...
                        return (
                          ...
                          >
                            ...
                            <Form.Item
                              ...
                              label={
                                <Text strong>
                                  What type of material are you adding?
                                </Text>
                              }
                              rules={[
                                {
                            ...
                            >
                              <Select
                                ...
                                onChange={(val) => {
                                  const values =
                                    inlineLessonForm.getFieldsValue(true) as {
                                      materials?: any[];
                                    };
                                  ...
                                  current = current.map((m: any, i: number) => {
                                    ...
                                    const pending =
                                      pendingMaterialFilesRef.current.get(i);
                                    if (
                                      pending &&
                                      (!m.file || !extractRawFile(m.file))
                                    ) {
                                      return {
                                        ...m,
                                        file: [
                                          {
                                            originFileObj: pending.file,
                                  ...
                                  if (
                                    name === 0 &&
                                    !inlineLessonForm.getFieldValue("title")
                                  ) {
                                    inlineLessonForm.setFieldValue("title", "");
                                  }
                                }}
                                options={MATERIAL_TYPE_OPTIONS.map(
                            ...
                            {type && (
                              <Form.Item noStyle shouldUpdate>
                                {() => {
                                  const t = inlineLessonForm.getFieldValue([
                                    "materials",
                                    name,
                                    "contentType",
                                  ]);
                                  const handleBeforeUpload = (file: File) => {
                                    ...
                                    if (!isLtG) {
                                      message.error(
                                        `${t === "Video" ? "Video" : "PDF"} must be smaller than ${t === "Video" ? `${VIDEO_SIZE_LIMIT_GB}GB` : `${PDF_SIZE_LIMIT_MB}MB`}!`,
                                      );
                                      return Upload.LIST_IGNORE;
                                    }
                                    const titleFromFile = cleanFileNameToTitle(
                                      file.name,
                                    );
                                    inlineLessonForm.setFieldValue(
                                  ...
                                  if (t === "Video" || t === "PDF") {
                                    ...
                                    const videoOrPdfField = (
                                      <Form.Item
                                        ...
                                        extra={
                                          t === "Video"
                                            ? `Drag and drop or click to browse. Max ${VIDEO_SIZE_LIMIT_GB}GB.`
                                            : `Drag and drop or click to browse. Max ${PDF_SIZE_LIMIT_MB}MB.`
                                        }
                                        ...
                                        rules={[
                                          {
                                            validator(_, value) {
                                              const hasInForm =
                                                !!extractRawFile(value);
                                              const hasInRef =
                                                !!pendingMaterialFilesRef.current.get(
                                                  name,
                                                )?.file;
                                              const materials =
                                                inlineLessonForm.getFieldValue(
                                        ...
                                        <div
                                          ...
                                          style={{
                                            ...(isDragOver && {
                                              ...
                                              background: "rgba(29, 155, 81, 0.06)",
                                            }),
                                          }}
                                          onDragOver={(e) => {
                                            e.preventDefault();
                                            setDragOverMaterialIndex(name);
                                          }}
                                          onDragLeave={() =>
                                          ...
                                          onDrop={(e) => {
                                            ...
                                            if (!sizeOk) {
                                              message.error(
                                                `File must be smaller than ${t === "Video" ? `${VIDEO_SIZE_LIMIT_GB}GB` : `${PDF_SIZE_LIMIT_MB}MB`}.`,
                                              );
                                              return;
                                            }
                                            const titleFromFile =
                                              cleanFileNameToTitle(file.name);
                                            pendingMaterialFilesRef.current.set(
                                              name,
                                              { file, fileName: file.name },
                                            );
                                            inlineLessonForm.setFieldsValue({
                                              materials: inlineLessonForm
                                            ...
                                            // CREATE and EDIT: file stored in state/ref; upload only on Save Lesson Group (edit) or Publish/Save Draft (create)
                                          }}
                                        >
                                          <Upload
                                            maxCount={1}
                                            ...
                                            onChange={(info) => {
                                              ...
                                              if (
                                                ...
                                                // CREATE and EDIT: file stored in ref; upload only on Save Lesson Group (edit) or Publish/Save Draft (create)
                                              }
                                            }}
                                          >
                                            {rawFile ? (
                                                ...
                                                <Space>
                                                  ...
                                                  <Button
                                                    ...
                                                    onClick={(e) => {
                                                      ...
                                                      pendingMaterialFilesRef.current.delete(
                                                        name,
                                                      );
                                                    }}
                                                  >
                                              ...
                                              <div className="flex flex-col items-center gap-2 text-gray-400 transition-colors group-hover:text-[#1D9B51]">
                                                ...
                                                <Text type="secondary" className="!text-inherit opacity-90">
                                                  Max{" "}
                                                  {t === "Video"
                                                    ? `${VIDEO_SIZE_LIMIT_GB}GB`
                                                    : `${PDF_SIZE_LIMIT_MB}MB`}
                                                </Text>
                                                {progress > 0 &&
                                                  progress < 100 && (
                                                    <Progress
                                                  ...
                                                  <Upload
                                                    ...
                                                    beforeUpload={(file) => {
                                                      if (
                                                        ...
                                                        message.error(
                                                          "Please upload a .vtt file",
                                                        );
                                                        return Upload.LIST_IGNORE;
                                                      }
                                                      uploadSubtitleImmediately(
                                                        name,
                                                        file,
                                                      );
                                                      return false;
                                                  ...
                                                  >
                                                    <div className="flex flex-col items-center gap-1 py-2 text-gray-400 transition-colors group-hover:text-[#1D9B51]">
                                                      ...
                                                      <Text className="text-sm !text-inherit">
                                                        .vtt subtitle (optional)
                                                      </Text>
                                                      <Text
                                                        type="secondary"
                                                        className="text-xs !text-inherit opacity-90"
                                                      >
                                                        Click or drop
                                                      </Text>
                                                      {subtitleProgress > 0 &&
                                        ...
                                        >
                                          <Input
                                            ...
                                            onBlur={async () => {
                                              ...
                                              const title =
                                                await fetchMaterialTitleFromUrl(
                                                  u,
                                                );
                            ...
                            >
                              {() => {
                                ...
                                return (
                                  ...
                                  >
                                    <Input
                                      placeholder="Add file/URL to auto-fill or type title"
                                      prefix={
                                        <EditOutlined
                                          style={{ color: "#bfbfbf" }}
                                        />
                ...
                {!assessmentMode && (
                  <>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Tag color="orange">
                        At least 1 question per lesson is mandatory
                      </Tag>
                    </div>
                        ...
                        <div className="space-y-4">
                          {fields.map(({ key, name, ...restField }) => (
                            <Card
                              size="small"
                              variant="outlined"
                              className="border-gray-100 bg-blue-50/10"
                              key={key}
                              title={
    ...
    <div className="py-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <Text type="secondary">
            Build your course structure. Add lessons inline below.
          </Text>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {lessons.map((lesson, index) => (
          <Card
            ...
            className={
              editingLessonId === lesson.id
                ? "mb-6 border-emerald-100 shadow-md"
          ...
          >
            {editingLessonId === lesson.id ? (
              <Form form={inlineLessonForm} layout="vertical" key={editingLessonId ?? "edit"}>
                {lessonFormBody}
                <div className="flex justify-end gap-3 mt-6">
                  <Button onClick={cancelLessonEdit}>Cancel</Button>
                  <Button
                    type="primary"
                    onClick={saveInlineLesson}
                    style={{
              ...
              <List
                ...
                renderItem={(m: LessonMaterial) => (
                  <List.Item
                    key={m.id}
                    className="py-1 px-4 border-b border-gray-50 last:border-b-0"
                  >
                    <Space>
                      {m.contentType === "Video" && (
        ...
        {isAddingLesson && !editingLessonId && (
          ...
          >
            <Form form={inlineLessonForm} layout="vertical" key="new">
              {lessonFormBody}
        ...
        {lessons.length === 0 && !isAddingLesson && (
          <Empty
            description="No lessons added yet"
      ...
      <div className="mb-6 bg-green-50 p-4 rounded-lg flex items-center gap-3 border border-green-100">
        ...
        <div>
          ...
          <Text type="secondary" style={{ fontSize: "13px" }}>
            Review all details below before making the course live.
          </Text>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <tbody className="divide-y divide-gray-100">
                ...
                <tr>
                  ...
                  <td className="py-2">
                    {form.getFieldValue("completionTimeValue")}{" "}
                    {form.getFieldValue("completionTimeUnit")}
                  </td>
                </tr>
                <tr>
            ...
            <div className="mt-4">
              ...
              <div className="max-h-40 overflow-y-auto border rounded p-2 bg-gray-50">
                <List
                  size="small"
                  dataSource={lessons}
                  renderItem={(item: Lesson, idx: number) => (
                    <List.Item>
                ...
                <div className="text-center p-2 bg-emerald-50 rounded">
                  ...
                  <div className="font-bold">
                    {lessons.reduce(
                      (acc, l) =>
                        acc +
                        l.materials.filter((m) => m.contentType === "Video")
                          .length,
                      0,
                    )}
                  </div>
                ...
                <div className="text-center p-2 bg-red-50 rounded">
                  ...
                  <div className="font-bold">
                    {lessons.reduce(
                      (acc, l) =>
                        acc +
                        l.materials.filter((m) => m.contentType === "YouTube")
                          .length,
                      0,
                    )}
                  </div>
                ...
                <div className="text-center p-2 bg-orange-50 rounded">
                  ...
                  <div className="font-bold">
                    {lessons.reduce(
                      (acc, l) =>
                        acc +
                        l.materials.filter((m) => m.contentType === "PDF")
                          .length,
                      0,
                    )}
                  </div>
  ...
  return (
    <Modal
      ...
      footer={
        <div className="flex flex-wrap justify-between items-center px-4 gap-y-3">
            ...
            {currentStep < 2 ? (
              ...
              <>
                {!initialData ? (
                  ...
                  /* Edit published: cannot go back to draft – Update course on the right with white text */
                  <>
                    <Button
                      loading={submittingStatus === "Archived"}
                      onClick={() => onFinalSubmit("Archived")}
                      icon={<ArchiveIcon />}
                    >
                      Archive
                    </Button>
    ...
    >
      ...
      <div className="wizard-content min-h-[400px]">
        {/* No key on Form: remounting with same form instance leaves store at old values (toggles OFF). We set values via setFieldsValue in effects. */}
        <Form
          form={form}
          layout="vertical"
          initialValues={{
        ...
        <div className="py-2">
          <div className="mb-4">
            <Text type="secondary" className="block mb-2">
              Uploading {materialSaveUploadProgress.totalFiles} material(s) to
              cloud storage...
            </Text>
            <Progress
              percent={Math.round(
                materialSaveUploadProgress.totalFiles > 0
                  ? Object.values(materialSaveUploadProgress.perFile).reduce(
                      (a, b) => a + b,
                      0,
                    ) / materialSaveUploadProgress.totalFiles
                  : 0,
              )}
              status="active"
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {Object.entries(materialSaveUploadProgress.fileNames).map(
              ([matIndexStr, fileName]) => {
                const matIndex = Number(matIndexStr);
                const percent =
                  materialSaveUploadProgress.perFile[matIndex] ?? 0;
                return (
