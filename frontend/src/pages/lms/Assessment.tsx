...
import {
  ...
  useUpdateAssessmentRemarksMutation,
...
const Assessment: React.FC = () => {
  ...
  const [submitAssessment, { isLoading: isSubmitting }] =
    useSubmitCourseAssessmentMutation();
  const [updateRemarks, { isLoading: isRemarksSubmitting }] =
    useUpdateAssessmentRemarksMutation();
  ...
  const closeRemarksAndShowResult = async (remarksToSave: string) => {
    if (!courseId) return;
    try {
      await updateRemarks({ courseId, remarks: remarksToSave }).unwrap();
    } catch (_) {
      // Non-blocking: still show result if remarks save fails
    }
    if (pendingResults) {
      setResults(pendingResults);
      setIsFinished(true);
      message.success(`Assessment Submitted. Score: ${pendingResults.score}%`);
    }
    ...
    return (
      <MainLayout>
        <Modal
          ...
          onCancel={() => closeRemarksAndShowResult("")}
          footer={[
            <Button
              key="skip"
              onClick={() => closeRemarksAndShowResult("")}
            ...
            <Button
              key="submit"
              type="primary"
              loading={isRemarksSubmitting}
              onClick={() => closeRemarksAndShowResult(remarks)}
            >
              Submit & view result
            </Button>,
          ]}
        ...
        >
          <Text type="secondary" className="block mb-3 text-sm">
            ...
            in the Standardized Assessment tab. You may skip.
          </Text>
          <TextArea
            rows={4}
            placeholder="e.g. Questions were clear, time was sufficient..."
            value={remarks}
  ...
  if (isFinished && results) {
    ...
    const formatAnswer = (a: string | string[]) =>
      ...
      <MainLayout>
        <div className="p-6 md:p-8 bg-[#f8f9fa] min-h-[calc(100vh-64px)]">
          <div className="max-w-3xl mx-auto">
            <Card
              bordered={false}
              className="text-center shadow-sm rounded-xl overflow-hidden p-8 mb-6"
            >
              <Avatar
                size={64}
              ...
              <Row gutter={24} justify="center" className="mb-8">
                <Col span={10}>
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <Text
                      type="secondary"
                      className="uppercase text-[9px] font-bold block mb-1"
                    >
                      Your Score
                    </Text>
                    <Title
              ...
              <Space size="middle">
                <Button
                  type="primary"
                  onClick={() => navigate(`/lms/employee/course/${courseId}`)}
                >
                  Go to Course
                </Button>
            ...
            {questionResults.length > 0 && (
              <Card
                title={<Text strong>Answer breakdown</Text>}
                bordered={false}
                className="shadow-sm rounded-xl"
              >
                <div className="space-y-4">
                  {questionResults.map((r, idx) => {
                    const q = flatQuestions.find(
                      (qu: any) => qu.id === r.questionId,
                    );
                    return (
                      <div
                        key={r.questionId}
                      ...
                      >
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <Tag color={r.isCorrect ? "success" : "error"}>
                            {r.marksAwarded}/{r.marksTotal} marks
                          </Tag>
                        </div>
                        <div className="text-left space-y-1 text-sm">
                          <div>
                            <Text type="secondary">Your answer: </Text>
                            <Text>{formatAnswer(r.userAnswer) || "—"}</Text>
                          </div>
                          {!r.isCorrect && (
                            <div>
  ...
  const progress = Math.round(
    ((currentQuestionIdx + 1) / flatQuestions.length) * 100,
  );
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
            <div>
              ...
              <Text type="secondary" className="text-sm">
                Certification exam for {course.title}
              </Text>
            </div>

            <div
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg shadow-sm border border-gray-100 bg-white ${timeRemaining < 300 ? "text-red-500 font-bold" : "text-gray-500"}`}
            >
              <ClockCircleOutlined />
          ...
          <Row gutter={[24, 24]}>
              ...
              >
                <div className="p-2">
                  <div className="flex justify-between items-end mb-4">
                    ...
                    <Tag className="m-0 text-[10px] border-none bg-emerald-50 text-emerald-600">
                  ...
                  <div className="py-2">
                    <Title level={5} className="mb-6 leading-relaxed">
                        ...
                        <div className="space-y-2">
                          {(question.options || []).map(
                            (option: string, idx: number) => {
                              const isSelected =
                                currentQuestionAnswers.includes(option);
                              return (
                                <div
                                  key={idx}
                                  className={`p-4 min-h-[48px] rounded-lg border cursor-pointer transition-all touch-manipulation flex items-center ${isSelected ? "border-emerald-500 bg-emerald-50" : "border-gray-100 bg-white hover:bg-gray-50"}`}
                                  onClick={() =>
                                    handleAnswerSelect(
                                      question.id,
                                      option,
                                ...
                                >
                                  <Space size="middle">
                                    ...
                                    <Text
                                      className={`text-sm sm:text-base ${isSelected ? "text-emerald-600 font-medium" : "text-gray-600"}`}
                                    >
                                      {option}
                                    </Text>
                                  </Space>
                        ...
                        >
                          <div className="space-y-2">
                            {(
                              ...
                              <div
                                key={idx}
                                className={`p-4 min-h-[48px] rounded-lg border cursor-pointer transition-all touch-manipulation flex items-center ${currentQuestionAnswers[0] === option ? "border-emerald-500 bg-emerald-50" : "border-gray-100 bg-white hover:bg-gray-50"}`}
                                onClick={() =>
                                  handleAnswerSelect(question.id, option, false)
                                }
                              >
                                <Space size="middle">
                                  <Radio value={option} />
                                  <Text
                                    className={`text-sm sm:text-base ${currentQuestionAnswers[0] === option ? "text-emerald-600 font-medium" : "text-gray-600"}`}
                                  >
                                    {option}
                                  </Text>
                                </Space>
                  ...
                  <div className="flex justify-between gap-3 mt-10 pt-6 border-t border-gray-50 flex-wrap">
                    ...
                    <Button
                      ...
                      icon={
                        currentQuestionIdx === flatQuestions.length - 1 ? (
                          <CheckCircleOutlined />
                        ) : (
                          <RightOutlined />
                        )
                      ...
                      iconPosition="end"
                      className="bg-emerald-600 hover:bg-emerald-700 border-emerald-600 min-h-[44px] touch-target"
                    >
                      {currentQuestionIdx === flatQuestions.length - 1
                        ? "Finish Assessment"
                        : "Next Question"}
            ...
            <Col xs={24} lg={7}>
              ...
              >
                <div className="grid grid-cols-5 gap-2">
                  {flatQuestions.map((_: any, idx: number) => {
                    ...
                    const isAnswered =
                      selectedAnswers[flatQuestions[idx].id]?.length > 0;
                    return (
                      <Button
                        key={idx}
                        type={isCurrent ? "primary" : "default"}
                        size="small"
                        className={`h-8 p-0 text-xs font-bold border-none ${isCurrent ? "bg-emerald-600" : ""} ${!isCurrent && isAnswered ? "bg-emerald-50 text-emerald-600" : ""}`}
              ...
              <Alert
                message={
                  <Text strong className="text-[11px]">
                    Honor Code
                  </Text>
                }
                description={
                  <ul className="pl-4 list-disc text-[10px] m-0 space-y-1 mt-1 opacity-80">
                    <li>No refreshing during test.</li>
                    <li>Passing mark: {course.qualificationScore || 80}%.</li>
                  </ul>
                }
                type="info"
                showIcon
