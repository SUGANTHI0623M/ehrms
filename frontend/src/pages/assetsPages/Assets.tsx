...
const Assets = () => {
  ...
  const { data: staffData } = useGetStaffQuery({ limit: 50, status: 'Active', page: 1 }, {
    refetchOnMountOrArgChange: false,
    refetchOnFocus: false,
  });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                  ...
                  >
                    <Select 
                      ...
                      onChange={(value) => {
                        // Auto-populate location when branch is selected
                        if (value) {
                          const selectedBranch = branches.find(b => b._id === value);
                          if (selectedBranch && selectedBranch.address) {
                            const addressParts: string[] = [];
                            if (selectedBranch.address.street) {
                              addressParts.push(selectedBranch.address.street);
                            }
                            if (selectedBranch.address.city) {
                              addressParts.push(selectedBranch.address.city);
                            }
                            if (selectedBranch.address.state) {
                              addressParts.push(selectedBranch.address.state);
                            }
                            if (selectedBranch.address.pincode) {
                              addressParts.push(selectedBranch.address.pincode);
                            }
                            if (selectedBranch.address.country) {
                              addressParts.push(selectedBranch.address.country);
                            }
                            ...
                            // Clear location if branch has no address
                            form.setFieldsValue({ location: '' });
                          }
                        } else {
                          // Clear location if branch is cleared
                          form.setFieldsValue({ location: '' });
                        }
                      }}
                    >
                  ...
                  >
                    <Input placeholder="Location will be auto-filled from branch address" />
                  </Form.Item>
