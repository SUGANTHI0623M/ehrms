...
import { useGetAvailableTemplatesForRoundQuery } from "@/store/api/jobOpeningApi";
import {
...
const ManagerInterviewScreen = () => {
  ...
  const templateId =
    interviewData?.data?.interview?.templateId ||
  ...
  const { data: templateByIdData } = useGetInterviewTemplateByIdQuery(
    templateId || "",
    {
      skip: !templateId,
    },
  ...
  const fallbackTemplateId =
    !templateId && templatesByTypeData?.data?.templates?.[0]?._id;

  // Fetch full template details for fallback template
  const { data: fallbackTemplateData } = useGetInterviewTemplateByIdQuery(
    fallbackTemplateId || "",
    {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      await submitResponse({
        ...
        recommendation:
          data.recommendation === "SELECTED" ? "PROCEED" : "REJECT", // Map to API values
      }).unwrap();
    ...
    <main className="p-4">
      <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-muted-foreground">
                {interview.stageName ||
                  `Stage ${interview.jobStage || interview.round || 2}`}{" "}
                - Manager Interview
              </p>
            </div>
          </div>
        ...
        {hrResponse && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">HR Round Summary</CardTitle>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div>
                    <Badge
                      variant={
                        hrResponse.recommendation === "PROCEED"
                          ? "default"
                          : "destructive"
                      }
                      className="text-xs sm:text-sm"
                ...
                <CardContent className="pt-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        {existingResponse.finalDecision ||
      ...
      <CardContent className="pt-6 space-y-4">
        ...
        {!isReadOnly && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>
                Score (1-{question.maxScore || 5}, positive integers only)
              ...
              <Input
                ...
                {...register(`responses.${index}.score`, {
                  ...
                  max: {
                    ...
                    message: `Max score is ${question.maxScore || 5}`,
                  },
                  validate: (value) => {
                    if (value < 1) return "Score must be at least 1";
                    if (value > (question.maxScore || 5))
                      return `Score cannot exceed ${question.maxScore || 5}`;
