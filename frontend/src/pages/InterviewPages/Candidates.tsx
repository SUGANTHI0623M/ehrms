...
const Candidates = () => {
  ...
  const [view, setView] = useState<"kanban" | "table" | "card" | "add">(
    "table",
  );
  ...
  const [page, setPage] = useState(() => {
    ...
    console.log("[Candidates] 🟦 Initial page state:", {
      pageParam,
      initialPage,
    });
    return initialPage;
  ...
  const lastDateRange = useRef<string | null>(
  ...
  const [selectedCandidateForInterview, setSelectedCandidateForInterview] =
    useState<{ id: string; name: string; round?: number } | null>(null);
  ...
  useEffect(() => {
    if (isInitialMount.current) {
      ...
      console.log("[Candidates] 🔵 Initial URL params:", {
        urlSearch,
        urlPage,
        urlLimit,
        currentPage: page,
      ...
      if (urlPage) {
        ...
        if (pageNum > 0) {
          console.log(
            "[Candidates] 🔵 Setting initial page from URL:",
            pageNum,
          );
          setPage(pageNum);
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      console.log(
        "[Candidates] 🟡 Debounced search executing, resetting to page 1",
      );
      lastSearch.current = search;
  ...
  useEffect(() => {
    ...
    if (lastPage.current !== 1) {
      console.log(
        "[Candidates] 🟢 Resetting page to 1 due to page size change",
      );
      lastPage.current = 1;
      setPage(1);
    }
  ...
  useEffect(() => {
    ...
    if (lastPage.current === page) {
      console.log("[Candidates] 🔴 Page effect skipped (page unchanged):", {
        page,
        lastPage: lastPage.current,
      });
      return;
    ...
    if (currentUrlPage === page) {
      console.log(
        "[Candidates] 🔴 Page already in sync with URL, updating ref only",
      );
    ...
    console.log("[Candidates] 🔴 Updating URL with new page:", {
    ...
    setSearchParams(
      (prevParams) => {
        ...
        console.log("[Candidates] 🔴 URL updated:", {
          old: prevParams.toString(),
          new: newParams.toString(),
        });
        return newParams;
      },
      { replace: true },
  ...
  const queryParams = useMemo(() => {
    ...
    if (view === "table") {
      ...
      // For Kanban and Card views, fetch candidates with pagination
      params.page = 1;
      params.limit = 50; // Reduced from 1000 - use pagination for better performance
    }
    ...
    ? [
        {
          ...
          value: statsData.data.stats.total,
          color: "text-gray-900",
        },
        {
        ...
        },
        {
          ...
          value: statsData.data.stats.documentVerification,
          color: "text-teal-600",
        },
        {
    ...
    <MainLayout>
      <main className="p-3 sm:p-4 overflow-x-hidden">
        <div className="space-y-6 w-full max-w-full">
          ...
          {/* FILTER SECTION - Status and date range (click "Filters" to open) */}
          {filterOpen && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    options={CANDIDATE_STATUS_FILTER_ORDER.map((status) => ({
                      value: status,
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-muted-foreground">Applied date range</label>
                  <RangePicker
                    className="w-full"
          ...
          <Card className="w-full overflow-hidden max-w-full">
              ...
              <div className="flex justify-between items-center w-full">
                ...
                <div className="flex gap-2 flex-wrap">
                  {statusFilter && (
                    ...
                    >
                      Status: {formatCandidateStatus(statusFilter)} ×
                    </Badge>
                  )}
                  {dateRange && (
                    <Badge
            ...
            <CardContent
              ...
              style={{ padding: view === "kanban" ? 0 : undefined }}
            >
              {/* CARD VIEW */}
              {view === "card" && (
                    ...
                    filteredCandidates.map((c) => (
                      ...
                      >
                        ...
                        {c.experience && c.experience.length > 0 && (
                          <p className="text-xs text-muted-foreground mt-1">
                            Experience: {c.experience[0].company} -{" "}
                            {c.experience[0].designation}
                          </p>
                        )}
                        {c.source === "REFERRAL" && c.referrerId && (
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            {typeof c.referrerId === "object" &&
                              c.referrerId && (
                                <span className="text-xs text-muted-foreground">
                                  by{" "}
                                  {c.referrerId.name ||
                                    `${(c.referrerId as any).firstName || ""} ${(c.referrerId as any).lastName || ""}`.trim()}
                                </span>
                              )}
                          </div>
                        )}
                              ...
                              onClick={async (e) => {
                                ...
                                if (
                                  ...
                                  try {
                                    ...
                                    message.success(
                                      "Candidate marked as ready for hiring",
                                    );
                                  } catch (err) {
                                    ...
                                    onClick={(e) => {
                                      ...
                                      switch (action.type) {
                                        ...
                                        case "START":
                                          ...
                                          navigate(
                                            `/interview/candidate/${c._id}/progress`,
                                          );
                                          break;
                                        ...
                                        case "GENERATE_OFFER":
                                          navigate(
                                            `/offer-letter/create?candidateId=${c._id}`,
                                          );
                                          break;
                                        ...
                                        case "BACKGROUND_VERIFICATION":
                                          ...
                                          if (hasDocs) {
                                            navigate(
                                              `/interview/background-verification/${c._id}`,
                                            );
                                          } else {
                                            navigate(
                                              `/interview/candidate/${c._id}/progress`,
                  ...
                  {candidatesData?.data?.pagination && (
                    <Pagination
                      ...
                      onPageChange={(newPage) => {
                        console.log(
                          "[Candidates] 🎯 PAGINATION BUTTON CLICKED:",
                          {
                            from: page,
                            to: newPage,
                            lastPage: lastPage.current,
                    ...
                    >
                      {[
                        ...
                        const columnCandidates = (
                          ...
                          // eslint-disable-next-line @typescript-eslint/no-explicit-any
                          const currentRound =
                            (c as any).currentJobStage || c.currentRound || 0;
                          // eslint-disable-next-line @typescript-eslint/no-explicit-any
                          const bgStatus = (c as any).backgroundVerification
                            ?.status;
                          ...
                          switch (column.id) {
                            ...
                            case "offer":
                              ...
                              return (
                                status === "SELECTED" || status === "OFFER_SENT"
                              );
                            case "docs":
                        ...
                        const handleDrop = async (
                          ...
                          if (isInterviewColumn) {
                            ...
                            setSelectedCandidateForInterview({
                              ...
                              name: candidateName,
                              round:
                                targetColumn.round && targetColumn.round > 0
                                  ? targetColumn.round
                                  : 1,
                            });
                            setIsScheduleModalOpen(true);
                            ...
                            try {
                              ...
                              message.success(
                                `Candidate moved to ${targetColumn.title}`,
                              );
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`${column.color} text-white px-4 py-3 rounded-t-xl flex items-center justify-between`}
                            >
                  ...
                  {selectedCard === 4 && (
                    <>
                      ...
                      <div className="border rounded-lg p-4 text-sm space-y-2">
                        ...
                        <p>
                          <b>Primary Skill:</b> {formData.primarySkill}
                        </p>
                        ...
                        {formData.experience.company && (
                          <>
                            <p>
                              <b>Company:</b> {formData.experience.company}
                            </p>
                            {formData.experience.designation && (
                              <p>
                                <b>Designation:</b>{" "}
                                {formData.experience.designation}
                              </p>
                            )}
                            {formData.experience.period && (
                              <p>
                                <b>Period:</b> {formData.experience.period}
                              </p>
                            )}
                            {formData.experience.reasonForLeaving && (
                              <p>
                        ...
                        {formData.resumeFile && (
                          <p>
                            <b>Resume:</b> {formData.resumeFile.name}
                          </p>
                        )}
                      </div>
                            ...
                            try {
                              ...
                              if (formData.dateOfBirth)
                                candidateData.dateOfBirth =
                                  formData.dateOfBirth;
                              if (formData.gender)
                                candidateData.gender = formData.gender;
        ...
        <ScheduleInterviewModal
          ...
          onClose={() => {
            ...
            if (view === "kanban") {
              console.log(
                "[Kanban] Refetching candidates after interview modal closed",
              );
