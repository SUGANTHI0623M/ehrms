...
const InterviewSelection = ({ type, roundNumber }: InterviewSelectionProps) => {
  ...
  const [activeTab, setActiveTab] = useState<"selected" | "rejected">(
    "selected",
  );
  ...
  // Rejection reason modal (for final round / selection reject)
  ...
  const { data: allCandidatesData, isLoading: isLoadingAll } =
    useGetCandidatesQuery(
      {
  ...
  // Hook for "Round" view
  const {
    data: roundCandidatesData,
    isLoading: isLoadingRound,
    refetch: refetchRoundCandidates,
  } = useGetCandidatesForRoundQuery(
    {
  ...
  const { data: tabCandidatesData, isLoading: isLoadingTabCandidates } =
    useGetCandidatesQuery(
      {
        search: search || undefined,
  ...
  if (type === "progress") {
    ...
    candidates = allCandidates.filter((candidate: any) => {
      ...
      return [
        "INTERVIEW_SCHEDULED",
        "HR_INTERVIEW_IN_PROGRESS",
        "HR_INTERVIEW_COMPLETED",
        "MANAGER_INTERVIEW_IN_PROGRESS",
  ...
  const paginationData =
    type === "selected"
      ? tabCandidatesData?.data?.pagination
      : type === "round"
        ? roundCandidatesData?.data?.pagination
        : type === "progress"
          ? allCandidatesData?.data?.pagination
          : null;
  ...
  // Open rejection modal so admin must enter rejection reason (after final round)
  const handleRejectClick = (candidateId: string) => {
  ...
  // Confirm rejection with reason (for INTERVIEW_COMPLETED candidates)
  const handleConfirmReject = async () => {
    if (!rejectCandidateId || !rejectionReason.trim()) {
    ...
    const renderTableHeaders = () => (
      <TableHeader>
        <TableRow>
          ...
          {type === "selected" && (
            <TableHead>
              {activeTab === "selected" ? "Selected On" : "Rejected On"}
            </TableHead>
          )}
                ...
                <span className="text-sm text-muted-foreground">
                  {candidate.scheduledInterview ? (
                    <>
                      Scheduled:{" "}
                      {new Date(
                        candidate.scheduledInterview.interviewDate,
                      ).toLocaleDateString()}
                    </>
                  ) : (
                    "Not Scheduled"
                ...
                activeTab === "selected" ? (
                  ...
                  candidate.status === "INTERVIEW_COMPLETED" ? (
                    ...
                    <Button
                      ...
                      onClick={() =>
                        navigate(
                          `/offer-letter/create?candidateId=${candidate._id}`,
                        )
                      }
                      disabled={isUpdatingStatus}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (action.type === "SCHEDULE") {
                          ...
                          action.type === "VIEW_PROGRESS"
                        ) {
                          navigate(
                            action.path ||
                              `/interview/candidate/${candidate._id}/progress`,
                          );
                        } else if (action.path) {
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <Input
                placeholder="Search by name (e.g. SUJITHA B), email, or position..."
                value={search}
                onChange={(e) =>
            ...
            {type === "selected" ? (
              <Tabs
                value={activeTab}
                onValueChange={(val) =>
                  setActiveTab(val as "selected" | "rejected")
                }
                className="w-full"
              >
            ...
            {(paginationData ||
              (type === "progress" && allCandidatesData?.data) ||
              (type === "round" && roundCandidatesData?.data)) && (
              <div className="mt-6 pt-4 border-t">
                <Pagination
                  ...
                  pages={
                    ...
                    Math.ceil(
                      (paginationData?.total || candidates.length) / pageSize,
                    )
                  }
      ...
      {/* Reject Candidate – require rejection reason (after final round) */}
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="rejection-reason-selection">
                Rejection Reason <span className="text-destructive">*</span>
              </Label>
              <Textarea
                id="rejection-reason-selection"
            ...
            <div className="space-y-2">
              <Label htmlFor="rejection-notes-selection">
                Additional Notes (Optional)
              </Label>
              <Textarea
                id="rejection-notes-selection"
