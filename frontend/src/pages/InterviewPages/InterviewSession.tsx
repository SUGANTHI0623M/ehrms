...
import { useGetInterviewByIdQuery } from "@/store/api/interviewApi";
...
export const InterviewSession = ({ interviewId }: { interviewId: string }) => {
  ...
  useEffect(() => {
    if (
      interviewId &&
      ["1", "2", "3", "final"].includes(interviewId.toLowerCase())
    ) {
      console.error(
        "Invalid interviewId - this route is reserved for candidate selection, not interview execution",
      );
  ...
  const jobId = interview?.jobOpeningId
    ? typeof interview.jobOpeningId === "object"
      ? interview.jobOpeningId._id
  ...
  const isAssigned =
    currentRound?.assignedInterviewers?.some((id: any) => {
  ...
  const roleMatches =
    currentRound?.assignedRole === userRole ||
    (currentRound?.assignedRole === "Recruiter" &&
      (userRole === "HR" || userRole === "Senior HR")) ||
  ...
  useEffect(() => {
    if (existingResponse) {
      ...
      const initialResponses = questions.map((q: any, index: number) => ({
        questionId:
          q._id ||
          `q-${index}-${q.questionText?.substring(0, 20) || "question"}`,
  ...
  const onSubmit = async (data: any) => {
    ...
    if (invalidScores.length > 0) {
      toast.error(
        `Please provide valid scores for all questions. Scores must be positive and not exceed the maximum allowed.`,
      );
      return;
    }
    ...
    if (missingRequired.length > 0) {
      toast.error(
        `Please answer all required questions: ${missingRequired.map((q: any) => q.questionText).join(", ")}`,
      );
      return;
    }
    ...
    try {
      ...
      if (result.data.nextRoundUnlocked && result.data.nextInterview) {
        const nextInterviewId =
          typeof result.data.nextInterview === "string"
            ? result.data.nextInterview
            : (result.data.nextInterview as any)?._id ||
              result.data.nextInterview;
      ...
      const errorMessage =
        error?.data?.error?.message || "Failed to submit interview";
  ...
  if (isLoadingFlow) {
    return (
      <div className="text-center py-8">Loading interview questions...</div>
    );
  }
  ...
  const roundName =
    currentRound?.roundName || interview.stageName || `Round ${jobStage}`;
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm sm:text-base font-medium">{roundName}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {!isCompleted && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">Evaluation & Submission</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6 pt-3">
              ...
              <div className="flex justify-end pt-4 border-t">
                <Button
                  ...
                  disabled={
                    isSubmitting ||
                    questions.some((q: any, index: number) => {
      ...
      <CardContent className="pt-6 pb-6 space-y-4">
        ...
        <div className="space-y-2 pt-2">
          ...
          {question.questionType === "scenario" && (
            <>
              {question.evaluationCriteria && (
                <div className="rounded-md border bg-muted/50 p-3 text-sm text-muted-foreground mb-2">
                  <p className="font-medium text-foreground mb-1">Scenario:</p>
                  <p className="whitespace-pre-wrap">{question.evaluationCriteria}</p>
                </div>
              )}
              <Textarea
                {...register(`responses.${index}.answer`)}
                placeholder="Enter your answer to the scenario..."
                rows={5}
                disabled={isReadOnly}
                defaultValue={currentResponse.answer || ""}
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <div className="space-y-2">
              <Label className="text-sm font-medium">
                Score (1-{question.maxScore}, positive integers only){" "}
              ...
              <Input
                ...
                {...register(`responses.${index}.score`, {
                  ...
                  validate: (value) => {
                    ...
                    if (value > (question.maxScore || 100))
                      return `Score cannot exceed ${question.maxScore}`;
          ...
          <div className="space-y-2">
            ...
            {currentResponse.score !== undefined && (
              <div>
                ...
                <p>
                  {currentResponse.score} / {question.maxScore || 5}
                </p>
              </div>
            )}
