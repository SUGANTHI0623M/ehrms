...
const JobOpeningForm = () => {
  ...
  const [generateJobDescription, { isLoading: isGeneratingDescription }] = useGenerateJobDescriptionMutation();
  ...
  const { data: jobData, isLoading: isLoadingJob } = useGetJobOpeningByIdQuery(
    id || "",
    { skip: isCreateMode },
  );
  ...
  const { data: candidatesData, isLoading: isLoadingCandidates } =
    useGetCandidatesByJobIdQuery(id || "", { skip: !shouldFetchCandidates });
  ...
  const [createJobOpening, { isLoading: isCreating }] =
    useCreateJobOpeningMutation();
  const [updateJobOpening, { isLoading: isUpdating }] =
    useUpdateJobOpeningMutation();
  const [publishJobOpening, { isLoading: isPublishing }] =
    usePublishJobOpeningMutation();
  ...
  useEffect(() => {
    if (isCreateMode && nextJobCodeData?.data?.jobCode) {
      setFormData((prev) => ({
        ...prev,
        jobCode: nextJobCodeData.data.jobCode,
      }));
    }
    ...
    if (jobData?.data?.jobOpening && !isCreateMode) {
      ...
      setFormData({
        ...
        workplaceType:
          ...
          (job.location === undefined && !(job as any).city
            ? "Remote"
            : "On-site"),
        numberOfPositions: job.numberOfPositions || 1,
        educationalQualification: eduQual,
        experienceLevel:
          (job as any).experienceLevel || "Fresher / Experienced",
        minExperience:
          (job as any).minExperience !== undefined &&
            (job as any).minExperience !== null
            ? (job as any).minExperience
            : undefined,
        maxExperience:
          (job as any).maxExperience !== undefined &&
            (job as any).maxExperience !== null
            ? (job as any).maxExperience
            : undefined,
  ...
  const handleCreateDepartment = async () => {
    ...
    try {
      ...
      const errorMessage =
        error?.data?.error?.message || "Failed to create department";
  ...
  const handleSave = async (publish: boolean = false) => {
    ...
    if (!formData.branchId) {
      toast.error("Please select a job location/branch");
      return;
    }
    ...
    if (
      formData.minExperience === undefined ||
      formData.minExperience === null ||
      formData.maxExperience === undefined ||
      formData.maxExperience === null
    ) {
      toast.error("Min Experience (Years) and Max Experience (Years) are required");
      return;
    }
    if (
      Number(formData.maxExperience) < Number(formData.minExperience)
    ) {
      toast.error("Maximum experience must be greater than or equal to minimum experience");
      return;
    }

    if (
      formData.salaryRange.min !== undefined &&
      formData.salaryRange.max !== undefined &&
      Number(formData.salaryRange.min) >= Number(formData.salaryRange.max) &&
      Number(formData.salaryRange.max) > 0
    ) {
    ...
    try {
      ...
      if (isCreateMode) {
        ...
        toast.success(
          publish
            ? "Job opening created and published successfully"
        ...
        toast.success(
          publish
            ? "Job opening updated and published successfully"
      ...
      const errorMessage =
        error?.data?.error?.message || "Failed to save job opening";
  ...
  // Filter candidates based on search query (normalized so paste works like typing)
  const filteredCandidates = useMemo(() => {
    const searchNorm = (debouncedCandidateSearch || "").replace(/\s+/g, " ").trim().toLowerCase();
    if (!searchNorm) {
      return candidates;
    }
  ...
  const branchLogo =
    isViewMode && jobData?.data?.jobOpening
      ? (() => {
    ...
    <MainLayout>
      <main className="p-3 sm:p-4 md:p-6 max-w-7xl mx-auto">
        <div className="space-y-4 sm:space-y-6">
              ...
              <div className="flex gap-2">
                ...
                {isCreateMode && (
                  <Button
                    ...
                    disabled={
                      isCreating || isUpdating || formData.status === "INACTIVE"
                    }
                  >
                    Save & Publish
                  </Button>
          ...
          {isViewMode ? (
            <Tabs defaultValue="details" className="space-y-3 sm:space-y-4">
              <TabsList className="w-full sm:w-auto">
                ...
                >
                  ...
                  <span className="sm:hidden">Apps ({candidates.length})</span>
                </TabsTrigger>
              </TabsList>
              ...
              >
                {jobData?.data?.jobOpening &&
                  (() => {
                    const job = jobData.data.jobOpening;
                    const jobBranch =
                      typeof job.branchId === "object" ? job.branchId : null;
                    const branchName =
                      ...
                      "";
                    const branchCity =
                      jobBranch?.address?.city ||
                      jobBranch?.city ||
                      branches.find((b) => b._id === formData.branchId)?.address
                        ?.city ||
                      "";
                    const createdByName =
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setCandidateSearch((e.target.value || "").replace(/\s+/g, " "))
                            }
                            onPaste={(e) => {
                              const pasted = (e.clipboardData?.getData("text") ?? "").replace(/\s+/g, " ").trim();
                              e.preventDefault();
                              setCandidateSearch(pasted);
                            }}
                          />
                          {candidateSearch && (
                            <Button
                        ...
                        {filteredCandidates.length === 0 ? (
                          <div className="text-center py-8 sm:py-12">
                            <Search className="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-muted-foreground mb-3 sm:mb-4" />
                                      ...
                                      <TableCell>
                                        ...
                                        >
                                          ...
                                          <span className="sm:hidden">
                                            {candidate.status.split("_")[0]}
                                          </span>
                                        </Badge>
                                      </TableCell>
                                      <TableCell>
                                        {candidate.currentJobStage
                                          ? `Round ${candidate.currentJobStage}`
                                          : "N/A"}
                                      </TableCell>
            ...
            <>
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 min-[1401px]:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(val: any) =>
                          setFormData({ ...formData, workplaceType: val })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          {formData.educationalQualification &&
                            !QUALIFICATION_OPTIONS.includes(
                              formData.educationalQualification,
                            ) && (
                              <SelectItem
                                value={formData.educationalQualification}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 min-[1401px]:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={formData.salaryRange.salaryType}
                        onValueChange={(val: any) =>
                          setFormData({
                            ...formData,
                            salaryRange: {
              ...
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Address is automatically populated from the selected branch
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle>
                      Job Requirements & Description Information
                    </CardTitle>
                    {!isViewMode && (
                      <Button
                        type="button"
                        ...
                        onClick={async () => {
                          ...
                          try {
                            toast.loading("Generating job description with AI...", { id: "ai-generate" });
                            const result = await generateJobDescription({
                              ...
                              location: formData.workplaceType === "Remote" ? "Remote" : formData.location || formData.city || undefined,
                              workplaceType: formData.workplaceType || undefined,
                              educationalQualification: formData.educationalQualification || undefined,
                            ...
                            setFormData((prev) => ({
                              ...
                              keyResponsibilities: result.data.keyResponsibilities || prev.keyResponsibilities,
                              requirements: result.data.requirements && result.data.requirements.length > 0 
                                ? result.data.requirements 
                                : prev.requirements,
                              skills: result.data.skills && result.data.skills.length > 0
                                ? [...new Set([...prev.skills, ...result.data.skills])]
                                : prev.skills,
                              benefits: result.data.benefits || prev.benefits,
                            }));
                            
                            toast.success("Job description, benefits, and all fields generated successfully!", { id: "ai-generate" });
                          } catch (error: any) {
                            toast.error(error?.data?.error?.message || "Failed to generate job description", { id: "ai-generate" });
                          }
                        }}
                      ...
                      {!isViewMode && (
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              toast.loading("Generating job description...", { id: "ai-desc" });
                              const result = await generateJobDescription({
                                role: formData.title,
                                skills: formData.skills.length > 0 ? formData.skills : undefined,
                                department: formData.department || undefined,
                              ...
                              toast.success("Generated successfully!", { id: "ai-desc" });
                            } catch (error: any) {
                              toast.error(error?.data?.error?.message || "Failed to generate", { id: "ai-desc" });
                            }
                          }}
                          disabled={isGeneratingDescription || !formData.title}
                          className="gap-2"
