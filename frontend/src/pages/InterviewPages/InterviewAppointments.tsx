...
const InterviewAppointments = () => {
  ...
  const { data: interviewData } = useGetInterviewByIdQuery(
    interviewIdForReschedule || "",
    {
  ...
  const { data: jobDataForReschedule } = useGetJobOpeningByIdQuery(
    jobIdForReschedule || "",
    {
      skip: !jobIdForReschedule,
    },
  );
  const { data: flowDataForReschedule } = useGetJobInterviewFlowQuery(
    jobIdForReschedule || "",
    {
      skip: !jobIdForReschedule,
    },
  ...
  const rescheduleRoundDetails = useMemo(() => {
    ...
    const roundNumber =
      selectedInterviewForReschedule?.jobStage ||
      selectedInterviewForReschedule?.round ||
      1;
    const round = flowDataForReschedule.data.job.interviewRounds.find(
    ...
    const interviewerId =
      round.assignedInterviewers && round.assignedInterviewers.length > 0
  ...
  const rescheduleInterviewer = useMemo(() => {
    if (
      !rescheduleRoundDetails?.assignedInterviewerId ||
      !usersData?.data?.users
    )
      return null;
    return usersData.data.users.find(
      (u: any) => u._id === rescheduleRoundDetails.assignedInterviewerId,
    );
  ...
  useEffect(() => {
    if (
      ...
      selectedInterviewForReschedule.interviewDate &&
      selectedInterviewForReschedule.interviewTime
    ) {
      try {
        const interviewDate = new Date(
          selectedInterviewForReschedule.interviewDate,
        );
        const [hours, minutes] = selectedInterviewForReschedule.interviewTime
          .split(":")
          .map(Number);
  ...
  useEffect(() => {
    if (selectedDate && calendarData?.data) {
      ...
      console.log(
        ...
        Object.keys(calendarData.data.calendar || {}),
      );
    }
  ...
  const interviewerUsers = (usersData?.data?.users || []).filter(
    (user: any) => {
      ...
      const isEmployeeWithValidSubRole =
        userRole === "Employee" &&
        userSubRole &&
  ...
  const handleReschedule = async (
    ...
    try {
      ...
      if (!interviewerId || !interviewer) {
        toast.error(
          "No interviewer assigned for this round in the interview flow. Please configure the interview flow first.",
        );
        return;
      }
      ...
      toast.error(
        error?.data?.error?.message || "Failed to reschedule interview",
      );
    }
  };
  ...
  const searchNorm = (searchQuery || "").replace(/\s+/g, " ").trim().toLowerCase();
  const filteredAppointments = appointments.filter((apt) => {
    ...
    const aptId = (apt._id || "").toLowerCase();
    return (
      candidateName.includes(searchNorm) ||
      email.includes(searchNorm) ||
      aptId.includes(searchNorm)
    );
  });
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <p className="text-sm text-muted-foreground">
              Interviews are scheduled from the candidate progress page based on
          ...
          <div className="grid grid-cols-1 min-[1301px]:grid-cols-3 gap-6">
            {/* TABLE */}
            <Card className="min-[1301px]:col-span-2">
              <CardHeader>
                    ...
                    <TableBody>
                      {isLoadingAppointments ? (
                        ...
                        filteredAppointments.map((apt) => {
                          ...
                          if (!interviewerName || interviewerName === "N/A") {
                            interviewerName =
                              apt.interviewerEmail || "Not Assigned";
                          }
                              ...
                              <TableCell className="text-center">
                                <div className="flex justify-center gap-2">
                                  <Button
                                    ...
                                    onClick={() => {
                                      const candidateId =
                                        getCandidateIdFromInterview(apt);
                                      if (candidateId) {
                                        navigate(
                                          `/interview/candidate/${candidateId}/progress`,
                                        );
                                      }
                                    }}
                                      ...
                                      <Button
                                        ...
                                        onClick={() => {
                                          ...
                                          if (candidateId) {
                                            ...
                                            console.error(
                                              "Cannot navigate: Invalid candidateId for interview",
                                              apt._id,
                                            );
                ...
                {appointmentsData?.data && (
                  <div className="mt-6 pt-4 border-t">
                    <Pagination
                      ...
                      pageSize={pageSize}
                      total={
                        appointmentsData.data.pagination?.total ||
                        appointmentsData.data.interviews?.length ||
                        0
                      }
                      pages={
                        appointmentsData.data.pagination?.pages ||
                        Math.ceil(
                          (appointmentsData.data.pagination?.total ||
                            appointmentsData.data.interviews?.length ||
                            0) / pageSize,
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-3">
                    {(() => {
                      ...
                      const selectedDateKeyUTC = selectedDate
                        .toISOString()
                        .split("T")[0];
                      ...
                      if (
                        ...
                        selectedDateInterviews =
                          calendarData.data.interviews.filter(
                            (interview: any) => {
                              const interviewDate = new Date(
                        ...
                        if (selectedDateInterviews.length === 0) {
                          ...
                          const selectedUTCKey = selectedUTC
                            .toISOString()
                            .split("T")[0];
                          ...
                          selectedDateInterviews =
                            calendarData.data.interviews.filter(
                              (interview: any) => {
                                ...
                                const interviewUTCKey = interviewDate
                                  .toISOString()
                                  .split("T")[0];
                      ...
                      const sortedInterviews = [...selectedDateInterviews].sort(
                        (a, b) => {
              ...
              <div className="space-y-4 py-2 sm:py-4">
                ...
                {isMobile ? (
                  <>
                    <div>
                      ...
                      <DatePicker
                        ...
                        defaultValue={
                          rescheduleDate ? dayjs(rescheduleDate) : undefined
                        }
                        format="YYYY-MM-DD"
                        onChange={(date) => {
                          if (date) {
                  ...
                  <div>
                    ...
                    <Popover
                      open={isReschedulePopoverOpen}
                      onOpenChange={setIsReschedulePopoverOpen}
                    >
                      ...
                      <PopoverContent
                        className="w-[calc(100vw-2rem)] max-w-[600px] lg:max-w-[700px] p-0 bg-white"
                        align="start"
                        sideOffset={4}
                        side="left"
                        style={{ zIndex: 10000 }}
                      >
                        ...
                        >
                          <div
                            className="p-3 md:p-4 flex-shrink-0 overflow-y-auto"
                            style={{ maxHeight: "85vh" }}
                          >
                                ...
                                onSelect={(date) => {
                                  if (date) {
                                    ...
                                    if (
                                      ...
                                      if (
                                        newDate.getTime() < minTime.getTime()
                                      ) {
                                        newDate.setHours(
                                          minTime.getHours(),
                                          minTime.getMinutes(),
                  ...
                  <div className="p-3 bg-slate-50 rounded-md border space-y-2">
                    {rescheduleRoundDetails ? (
                      <>
                        <div>
                          ...
                          <span className="ml-2 font-medium text-sm">
                            {rescheduleRoundDetails.roundName ||
                              `Round ${rescheduleRoundDetails.roundNumber}`}
                          </span>
                        </div>
                          ...
                          <div>
                            ...
                            <div className="mt-1">
                              ...
                              <span className="ml-2 text-xs text-muted-foreground">
                                {rescheduleInterviewer.role === "Employee" &&
                                rescheduleInterviewer.subRole
                                  ? `${rescheduleInterviewer.role} (${rescheduleInterviewer.subRole})`
                                  : rescheduleInterviewer.role}
                                {rescheduleInterviewer.email &&
                                  ` • ${rescheduleInterviewer.email}`}
                              </span>
                            </div>
                          </div>
                      ...
                      <span className="text-sm text-muted-foreground">
                        {flowDataForReschedule
                          ? "Loading round details..."
                          : "No interview flow configured for this job"}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    Interviewer is assigned from the interview flow
                    configuration. To change, update the interview flow for this
                    job.
                  </p>
                </div>
              </div>
