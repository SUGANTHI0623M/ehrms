...
const InterviewProgress = () => {
  ...
  const candidateId =
    candidateIdParam && isValidObjectId(candidateIdParam)
      ? candidateIdParam
      : null;
  ...
  const { data: candidateData, refetch: refetchCandidate } =
    useGetCandidateByIdQuery(candidateId || "", {
      skip: !candidateId,
    });

  const { data: interviewsData } = useGetCandidateInterviewsQuery(
    candidateId || "",
    { skip: !candidateId },
  );
  ...
  const { data: bgvData } = useGetBackgroundVerificationDetailsQuery(
    candidateId || "",
    {
  ...
  const { data: jobOpeningData } = useGetJobOpeningByIdQuery(jobId || "", {
    skip: !jobId,
  });

  // Get Interview Flow (InterviewTemplate) - primary source of truth
  const { data: interviewFlowData } = useGetInterviewFlowByJobIdQuery(
    jobId || "",
    {
      skip: !jobId,
    },
  ...
  const visibleRounds = enabledRounds.filter((roundConfig: any) => {
    ...
    if (!isPrivileged && interviewId) {
      const roundProgress = progress.find(
        (p) => p.round === roundConfig.roundNumber,
      );
  ...
  return (
    <>
      <div className="space-y-6">
              ...
              {(isHR || isManager) &&
                ...
                !candidate?.hiredForOtherJob && (
                  <Button
                    onClick={() =>
                      navigate(
                        `/offer-letter/create?candidateId=${candidateId}`,
                      )
                    }
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-green-800 dark:text-green-200">
                      This candidate has been successfully converted to an
                      employee. Offer letter and onboarding details are no
                      longer available for this candidate.
                    </p>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-orange-800 dark:text-orange-200">
                      This candidate has already been hired and converted to an
                      employee for{" "}
                      <strong>{candidate.hiredForOtherJob.jobTitle}</strong>.
                      They cannot proceed with the interview process for this
                      job. Please contact HR if this is an error.
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <InterviewStatusTimeline timeline={progressData?.data?.timeline} />
              ...
              <div className="space-y-4">
                {visibleRounds.length > 0
                  ? visibleRounds.map((roundConfig: any) => {
                      ...
                      console.log(
                        `[InterviewProgress] Round ${roundConfig.roundNumber}:`,
                        {
                          roundConfig,
                          roundProgress,
                      ...
                      const isLocked =
                        ...
                        !progress.some(
                          (p) =>
                            p.round === roundConfig.roundNumber - 1 &&
                            p.isCompleted &&
                            p.recommendation === "PROCEED",
                        );

                      // Check if user can access this round
                      const canAccess =
                        roundProgress?.canAccess ||
                        currentUser?.role === "Super Admin" ||
                        currentUser?.role === "Admin" ||
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 space-y-2 w-full">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-muted-foreground">
                                    ...
                                    {roundConfig.assignedRole || "Interview"}{" "}
                                    Round
                                    {roundConfig.assignedInterviewers &&
                                      roundConfig.assignedInterviewers.length >
                                        0 && (
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm p-3 bg-muted/50 rounded-lg">
                                  ...
                                  {roundProgress.completedAt && (
                                    <div>
                                      ...
                                      <span>
                                        {formatDate(roundProgress.completedAt)}
                                      </span>
                                    </div>
                                  )}
                            ...
                            {isLocked && (
                              <p className="text-sm text-muted-foreground">
                                This round will unlock after the previous round
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                Round {round.round} - Assigned to:{" "}
                                {round.assignedRole || "N/A"}
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                ...
                {enabledRounds.length === 0 && progress.length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    No interview rounds configured for this job. Please
                    configure interview rounds first.
                  </div>
                )}
              </div>
            </CardContent>
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-muted rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {(jobOpeningData?.data?.jobOpening as any)?.department ||
                      "N/A"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {jobOpeningData?.data?.jobOpening?.title ||
                      candidate.position}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium">
                    {candidate.expectedJoining
                      ? formatDate(candidate.expectedJoining)
                      : "Not Set"}
                  </p>
                </div>
              </div>
      ...
      <Dialog
        open={!!logsRound}
        onOpenChange={(open) => !open && setLogsRound(null)}
      >
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-muted/50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {logsRound.completedAt
                      ? formatDate(logsRound.completedAt)
                      : "N/A"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="font-medium text-lg">
                    {logsRound.overallScore !== undefined
                      ? logsRound.overallScore.toFixed(1)
                      : "N/A"}
                  </p>
                </div>
                <div>
                  ...
                  <div className="mt-1">
                    <Badge
                      variant={
                        logsRound.recommendation === "PROCEED"
                          ? "default"
                          : "destructive"
                      }
                    >
              ...
              {logsRound.responses && logsRound.responses.length > 0 ? (
                <div className="space-y-4">
                  <h4 className="font-semibold text-sm border-b pb-2">
                    Questions & Answers ({logsRound.responses.length} question
                    {logsRound.responses.length !== 1 ? "s" : ""})
                  </h4>
                  <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          {response.remarks && (
                            <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 p-3 rounded-lg text-sm mt-2">
                              ...
                              <p className="text-blue-900 dark:text-blue-100 mt-1 whitespace-pre-wrap">
                                {response.remarks}
                              </p>
                            </div>
                          )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setLogsRound(null)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
