...
const SalaryStructureView = ({ employeeId: propEmployeeId }: SalaryStructureViewProps = {}) => {
  ...
  // Calculate date range for selected month - use current date for current month
  const currentDate = new Date();
  const isCurrentMonth = currentDate.getMonth() === selectedMonth - 1 && currentDate.getFullYear() === selectedYear;
  const startDate = new Date(selectedYear, selectedMonth - 1, 1);
  const endDate = isCurrentMonth ? currentDate : new Date(selectedYear, selectedMonth, 0, 23, 59, 59);
  const startDateStr = format(startDate, 'yyyy-MM-dd');
  const endDateStr = format(endDate, 'yyyy-MM-dd');

  // Fetch attendance for selected month - only till current date for current month
  ...
  // Fetch holidays for current year (limited - holidays are usually < 20 per year)
  ...
  // Calculate present days (full days + half days) - consistent with other modules
  ...
  const isCurrentMonthForWorkingDays = currentDate.getMonth() + 1 === selectedMonth && currentDate.getFullYear() === selectedYear;
  const workingDaysEndDate = isCurrentMonthForWorkingDays ? currentDate : undefined;
  ...
  const workingDaysInfo = calculateWorkingDays(
    ...
    monthHolidays, 
    weeklyOffPattern,
    workingDaysEndDate // Pass current date for current month
  );
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                      ...
                      <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded-lg">
                        <div className="text-xs text-muted-foreground">Half Day Present</div>
                    ...
                    <div className="p-4 bg-muted/50 rounded-lg space-y-2">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="pt-2 border-t">
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Based on {presentDays % 1 === 0 ? presentDays : presentDays.toFixed(1)} present days
                          </p>
                        </div>
                      </div>
                    </div>
