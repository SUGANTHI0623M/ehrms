...
const Staff = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: "",
    confirmPassword: "",
    joiningDate: format(new Date(), "yyyy-MM-dd"), // Default to today's date
    shiftName: "",
  ...
  // Keep page and pageSize in sync with URL (e.g. browser back/forward, or when URL is updated by effect)
  useEffect(() => {
    const pageFromUrl = Number(searchParams.get("page")) || 1;
    const limitFromUrl = Number(searchParams.get("limit")) || 20;
    setCurrentPage((p) => (pageFromUrl !== p ? pageFromUrl : p));
    setPageSize((s) => (limitFromUrl !== s ? limitFromUrl : s));
  }, [searchParams]);

  // Normalize search: trim and collapse multiple spaces (paste "  John   Doe  " works; backend is case-insensitive)
  const normalizeSearch = (q: string) => q.trim().replace(/\s+/g, " ");

  // Debounce search input - only reset page to 1 when search term actually changed (not on mount)
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      ...
      // Only reset to first page when user changed the search term (avoids resetting after going to page 2)
      const searchChanged = normalized !== debouncedSearchQuery;
      if (searchChanged) {
        setCurrentPage(1);
        setSearchParams((prevParams) => {
          const newParams = new URLSearchParams(prevParams);
          if (normalized) {
            newParams.set("search", normalized);
          } else {
        ...
        // Keep search in URL in sync even when page wasn't reset
        setSearchParams((prevParams) => {
          const newParams = new URLSearchParams(prevParams);
          if (normalized) {
            newParams.set("search", normalized);
  ...
  const { data: staffData, isLoading: isLoadingStaff } = useGetStaffQuery({
    ...
    page: currentPage,
    limit: pageSize // Use standard pagination
  });
              ...
              {canCreate && (
                ...
                <DialogContent className="max-w-2xl w-[95%]">
                  ...
                  <form onSubmit={async (e) => {
                    ...
                    const limits = phoneUtils.getLimits(formData.countryCode || "91");
                    const phoneDigits = (formData.phone || "").replace(/\D/g, "");
                    if (!formData.phone || !phoneDigits.length) {
                      message.error("Please enter a phone number");
                      return;
                    }
                    if (limits) {
                      if (!phoneUtils.validateMobileNumber(formData.countryCode || "91", formData.phone)) {
                        message.error(`Phone number must be ${limits.min}–${limits.max} digits for the selected country`);
                        return;
                      }
                    } else {
                      if (phoneDigits.length < 8 || phoneDigits.length > 15) {
                        message.error("Phone number must be 8–15 digits");
                        return;
                      }
                    }

                    // Validate alternative phone if provided (same country rules)
                    if (formData.alternativePhone && formData.alternativePhone.trim()) {
                      const altDigits = formData.alternativePhone.replace(/\D/g, "");
                      if (limits) {
                        if (!phoneUtils.validateMobileNumber(formData.countryCode || "91", formData.alternativePhone)) {
                          message.error(`Alternative phone must be ${limits.min}–${limits.max} digits for the selected country`);
                          return;
                        }
                      } else if (altDigits.length < 8 || altDigits.length > 15) {
                        message.error("Alternative phone must be 8–15 digits");
                        return;
                      }
                    }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
                          ...
                          <PopoverTrigger asChild>
                            ...
                            >
                              {formData.countryCode
                                ? getCountryOptions().find((o) => o.value === formData.countryCode)?.label ?? `+${formData.countryCode}`
                                : "Select country code..."}
                                  ...
                                  {getCountryOptions().map((option) => (
                                    <CommandItem
                                      ...
                                      onSelect={() => {
                                        ...
                                        let phone = (formData.phone || '').replace(/\D/g, '').slice(0, maxLen);
                                        let alt = (formData.alternativePhone || '').replace(/\D/g, '').slice(0, maxLen);
                                        setFormData({
                                          ...formData,
                                          countryCode: option.value,
                                          phone,
                                          alternativePhone: alt || undefined
                                        });
                                        setCountryCodeOpen(false);
                                      }}
                                    >
                                      <Check
                                        className={cn(
                                          "mr-2 h-4 w-4",
                                          formData.countryCode === option.value ? "opacity-100" : "opacity-0"
                                        )}
                                      />
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          placeholder={(() => {
                            const limits = phoneUtils.getLimits(formData.countryCode || "91");
                            return limits ? `Enter ${limits.min}–${limits.max} digits` : "Enter phone number";
                          })()}
                          required
                          maxLength={phoneUtils.getLimits(formData.countryCode || "91")?.max ?? 15}
                        />
                        <p className="text-xs text-muted-foreground">
                          {formData.countryCode && (() => {
                            const limits = phoneUtils.getLimits(formData.countryCode);
                            return limits ? `${limits.min}–${limits.max} digits for selected country` : "8–15 digits";
                          })()}
                        </p>
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          placeholder={(() => {
                            const limits = phoneUtils.getLimits(formData.countryCode || "91");
                            return limits ? `Enter ${limits.min}–${limits.max} digits (optional)` : "Enter phone number (optional)";
                          })()}
                          maxLength={phoneUtils.getLimits(formData.countryCode || "91")?.max ?? 15}
                        />
                        <p className="text-xs text-muted-foreground">
                          {formData.countryCode && (() => {
                            const limits = phoneUtils.getLimits(formData.countryCode);
                            return limits ? `Optional - ${limits.min}–${limits.max} digits for selected country` : "Optional - 8–15 digits";
                          })()}
                        </p>
                      </div>
                      <div className="space-y-1">
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (result.data.failedItems.length > 0) {
                            const failedList = result.data.failedItems
                              .map((f: { row: number; error: string }) => `Row ${f.row}: ${f.error}`)
                              .join('\n');
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1 sm:max-w-md">
                    ...
                    <Input
                      placeholder="Search by name, ID, or email (case-insensitive)..."
