  ...
  useEffect(() => {
    if (allUsersData?.data?.users && selectedRoles.length > 0) {
      ...
      const filtered = allUsersData.data.users.filter((user: any) => {
        // Match main role
        if (selectedRoles.includes(user.role)) return true;

        // Match sub-role (e.g., Senior HR, Junior HR)
        if (user.subRole && selectedRoles.includes(user.subRole)) return true;

        // Special case: "HR" might be a generic filter for any HR sub-role
        if (
          selectedRoles.includes("HR") &&
          user.subRole?.toLowerCase().includes("hr")
        )
          return true;
    ...
    <MainLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            onClick={() => navigate("/hrms-geo/tasks/settings")}
          >
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Select roles to filter users. Users from selected roles will be
