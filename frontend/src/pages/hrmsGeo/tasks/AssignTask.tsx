...
const AssignTask = () => {
  ...
  const getActiveTab = () => {
    ...
    // Default to assign if path doesn't match any specific tab
    return "assign";
  };
  ...
  const { data: taskCustomFieldsResponse, isLoading: isLoadingCustomFields } =
    useGetTaskCustomFieldsQuery();
        ...
        return (
          <div key={field._id} className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(val) =>
                handleCustomFieldChange(fieldName, val === "true")
              }
            >
              <SelectTrigger className={error ? "border-red-500" : ""}>
                <SelectValue />
  ...
  const validateForm = () => {
    ...
    if (!formData.earliestCompletionDate) {
      ...
      // Validate that earliest completion date is not in the past
      const earliestDate = new Date(formData.earliestCompletionDate);
      const today = new Date();
      today.setHours(0, 0, 0, 0); // Reset time to start of day for accurate comparison
      earliestDate.setHours(0, 0, 0, 0);
      
      if (earliestDate < today) {
        newErrors.earliestCompletionDate =
          "Earliest completion date cannot be in the past";
      }
    }
    if (!formData.latestCompletionDate) {
      newErrors.latestCompletionDate = "Latest completion date is required";
    } else {
      // Validate that latest completion date is not in the past
      const latestDate = new Date(formData.latestCompletionDate);
      const today = new Date();
      today.setHours(0, 0, 0, 0); // Reset time to start of day for accurate comparison
      latestDate.setHours(0, 0, 0, 0);
      
      if (latestDate < today) {
        newErrors.latestCompletionDate =
          "Latest completion date cannot be in the past";
      }
    }
    if (formData.earliestCompletionDate && formData.latestCompletionDate) {
      if (
        new Date(formData.earliestCompletionDate) >
        new Date(formData.latestCompletionDate)
      ) {
        newErrors.latestCompletionDate =
          "Latest completion date must be after earliest completion date";
      }
    }
      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Create and assign new tasks to staff members
          </p>
        </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          errors.earliestCompletionDate ? "border-red-500" : ""
                        }
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          errors.latestCompletionDate ? "border-red-500" : ""
                        }
                      />
                  ...
                  <div className="p-4 bg-muted rounded-md">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      This task ID will be automatically generated when you
                      assign the task
                    </p>
                  </div>
