...
import type { CustomerDataField } from "@/store/api/settingsApi";

const CustomerDataFields = () => {
  const navigate = useNavigate();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingField, setEditingField] = useState<CustomerDataField | null>(null);
  const [formData, setFormData] = useState({
    name: "",
    label: "",
    type: "text" as CustomerDataField['type'],
    placeholder: "",
    required: false,
    options: [] as string[],
    order: 0
  ...
  const { data: fieldsData, isLoading, refetch } = useGetCustomerDataFieldsQuery();
  const [createField] = useCreateCustomerDataFieldMutation();
  const [updateField] = useUpdateCustomerDataFieldMutation();
  const [deleteField] = useDeleteCustomerDataFieldMutation();

  const fields = fieldsData?.data?.fields || [];

  const handleOpenModal = (field?: CustomerDataField) => {
    if (field) {
      ...
      <main className="ml-64 mt-16 p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center gap-4">
            <Button variant="ghost" onClick={() => navigate('/hrms-geo/customers/settings')}>
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back
            </Button>
            <div>
              <h1 className="text-3xl font-bold text-foreground">Customer Data Fields</h1>
              <p className="text-muted-foreground mt-1">Manage custom fields for customer data collection</p>
            </div>
          </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={formData.type}
                onValueChange={(val) => setFormData(prev => ({ ...prev, type: val as CustomerDataField['type'] }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
