...
const OnboardingDocuments = () => {
  ...
  const handleFileUpload = async (document: OnboardingDocument, file: File) => {
    ...
    // Set the uploading document ID to show loading only for this specific document
    setUploadingDocumentId(document._id);
    ...
    try {
      ...
      // Clear the uploading document ID after upload completes
                      ...
                      {doc.url ? (
                          ...
                          {doc.status !== DOCUMENT_STATUS.COMPLETED && (
                            <label>
                              <input
                                ...
                                disabled={uploadingDocumentId === doc._id}
                              />
                              <Button size="sm" variant="outline" disabled={uploadingDocumentId === doc._id} asChild>
                                <span>
                                  {uploadingDocumentId === doc._id ? (
                                    <>
                        ...
                        <label>
                          <input
                            ...
                            disabled={uploadingDocumentId === doc._id}
                          />
                          <Button size="sm" disabled={uploadingDocumentId === doc._id} asChild>
                            <span>
                              {uploadingDocumentId === doc._id ? (
                                <>
                                  <Upload className="w-4 h-4 mr-1 animate-pulse" />
