      ...
      <MainLayout>
        <div className="p-3 sm:p-4 md:p-6">
          <div className="text-center py-8 sm:py-12">
            ...
            <p className="mt-4 text-sm sm:text-base text-muted-foreground">Loading job openings...</p>
          </div>
        </div>
      </MainLayout>
    );
  ...
  return (
    <MainLayout>
      <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6">
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground">Job Openings</h1>
          <p className="text-sm sm:text-base text-muted-foreground mt-1">
            Browse available positions and apply
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
          {jobVacancies.length > 0 ? (
                ...
                <Card key={job._id} className="hover:shadow-lg transition-shadow flex flex-col h-full">
                  <CardHeader className="pb-3 sm:pb-4">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-2">
                      ...
                      <Badge variant={job.status === 'ACTIVE' ? 'default' : 'secondary'} className="text-xs sm:text-sm whitespace-nowrap self-start sm:self-auto">
                  ...
                  <CardContent className="flex-1 flex flex-col">
                    <div className="space-y-2 sm:space-y-3 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-xs sm:text-sm text-muted-foreground">
                        ...
                        <span className="truncate">Updated {formatDistanceToNow(new Date(job.latestUpdate), { addSuffix: true })}</span>
                      </div>
                      {job.employmentType && (
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 mt-4 pt-2 border-t">
                        ...
                        {buttonState.disabled ? (
                          <div className="flex-1 min-w-0">
                            <Badge 
                              variant="secondary" 
                              className="w-full justify-center text-xs sm:text-sm py-2 px-3 h-auto min-h-[2rem] flex items-center gap-1.5 sm:gap-2"
                            >
                              <CheckCircle2 className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                              <span className="truncate text-center">{buttonState.text}</span>
                            </Badge>
                          </div>
                        ) : (
                          <Button
            ...
            <div className="col-span-full text-center py-8 sm:py-12">
              <p className="text-sm sm:text-base text-muted-foreground">No active job openings at the moment.</p>
        ...
        >
          <DialogContent className="max-w-[95vw] sm:max-w-2xl md:max-w-4xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
            <DialogHeader>
              <DialogTitle className="text-lg sm:text-xl">Application Form</DialogTitle>
