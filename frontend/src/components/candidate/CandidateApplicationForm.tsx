...
/** Parse yearOfPassing string (year only "2020", or date "2020-06-15", or from resume) to dayjs for DatePicker */
function parseYearOfPassingToDayjs(value: string | undefined): Dayjs | null {
  if (!value || typeof value !== "string") return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  // Full date (YYYY-MM-DD or ISO)
  const asDate = dayjs(trimmed, ["YYYY-MM-DD", "YYYY-M-D", "DD/MM/YYYY", "MM/DD/YYYY"], true);
  if (asDate.isValid()) return asDate;
  // Year only (4 digits)
  if (/^\d{4}$/.test(trimmed)) return dayjs(trimmed, "YYYY", true).startOf("year");
  // Try generic parse
  const fallback = dayjs(trimmed);
  return fallback.isValid() ? fallback : null;
}

/** Format dayjs for storage (YYYY-MM-DD). Empty for null. */
function formatYearOfPassingForStore(date: Dayjs | null): string {
...
function parseDateStringToDayjs(value: string | undefined): Dayjs | null {
  return parseYearOfPassingToDayjs(value);
}

/** Format stored date for display with full date (e.g. "15 Jun 2020") */
function formatDateDisplay(value: string | undefined): string {
  if (!value || typeof value !== "string") return "";
  const d = parseDateStringToDayjs(value);
  if (!d) return value;
    ...
    if (formData) {
      ...
      if (formData.education && formData.education.length > 0) {
        formValues.education = formData.education.map(edu => ({
          ...edu,
          yearOfPassing: parseYearOfPassingToDayjs(edu.yearOfPassing) || undefined,
        }));
      }
      ...
      if (formData.experience && formData.experience.length > 0) {
        formValues.experience = formData.experience.map(exp => ({
          ...exp,
          durationFrom: parseDateStringToDayjs(exp.durationFrom) || undefined,
          durationTo: parseDateStringToDayjs(exp.durationTo) || undefined,
        }));
      }
      ...
      if (formData.courses && formData.courses.length > 0) {
        formValues.courses = formData.courses.map(course => ({
          ...course,
          startDate: parseDateStringToDayjs(course.startDate) || undefined,
          completionDate: parseDateStringToDayjs(course.completionDate) || undefined,
        }));
      } else {
        formValues.courses = [];
      }
      ...
      if (formData.internships && formData.internships.length > 0) {
        formValues.internships = formData.internships.map(internship => ({
          ...internship,
          durationFrom: parseDateStringToDayjs(internship.durationFrom) || undefined,
          durationTo: parseDateStringToDayjs(internship.durationTo) || undefined,
        }));
      } else {
      ...
      if (result.success && result.data.parsedData) {
        ...
        if (parsed.education && Array.isArray(parsed.education) && parsed.education.length > 0) {
          updatedFormData.education = parsed.education.map((edu: any) => {
            const raw = edu.yearOfPassing != null ? String(edu.yearOfPassing).trim() : '';
            const parsedDate = parseYearOfPassingToDayjs(raw);
            const yearOfPassing = raw ? (parsedDate ? formatYearOfPassingForStore(parsedDate) : raw) : '';
            return {
        ...
        // Experience – normalize date strings (year or full date) to YYYY-MM-DD for date picker
        if (parsed.experience && Array.isArray(parsed.experience) && parsed.experience.length > 0) {
          updatedFormData.experience = parsed.experience.map((exp: any) => {
            const fromRaw = exp.durationFrom != null ? String(exp.durationFrom).trim() : '';
            const toRaw = exp.durationTo != null ? String(exp.durationTo).trim() : '';
            const fromParsed = parseDateStringToDayjs(fromRaw);
            const toParsed = parseDateStringToDayjs(toRaw);
            return {
              company: exp.company || '',
              role: exp.role || '',
              designation: exp.designation || '',
              durationFrom: fromRaw ? (fromParsed ? formatYearOfPassingForStore(fromParsed) : fromRaw) : '',
              durationTo: toRaw ? (toParsed ? formatYearOfPassingForStore(toParsed) : toRaw) : '',
        ...
        // Courses (for freshers) – normalize date strings
        if (parsed.courses && Array.isArray(parsed.courses) && parsed.courses.length > 0) {
          updatedFormData.courses = parsed.courses.map((course: any) => {
            const startRaw = course.startDate != null ? String(course.startDate).trim() : '';
            const endRaw = course.completionDate != null ? String(course.completionDate).trim() : '';
            const startParsed = parseDateStringToDayjs(startRaw);
            const endParsed = parseDateStringToDayjs(endRaw);
            return {
              courseName: course.courseName || '',
              institution: course.institution || '',
              startDate: startRaw ? (startParsed ? formatYearOfPassingForStore(startParsed) : startRaw) : undefined,
              completionDate: endRaw ? (endParsed ? formatYearOfPassingForStore(endParsed) : endRaw) : undefined,
        ...
        // Internships (for freshers) – normalize date strings
        if (parsed.internships && Array.isArray(parsed.internships) && parsed.internships.length > 0) {
          updatedFormData.internships = parsed.internships.map((internship: any) => {
            const fromRaw = internship.durationFrom != null ? String(internship.durationFrom).trim() : '';
            const toRaw = internship.durationTo != null ? String(internship.durationTo).trim() : '';
            const fromParsed = parseDateStringToDayjs(fromRaw);
            const toParsed = parseDateStringToDayjs(toRaw);
            return {
              ...
              durationFrom: fromRaw ? (fromParsed ? formatYearOfPassingForStore(fromParsed) : fromRaw) : '',
              durationTo: toRaw ? (toParsed ? formatYearOfPassingForStore(toParsed) : toRaw) : undefined,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <DatePicker
                      ...
                      picker="date"
                      format="DD/MM/YYYY"
                      value={parseYearOfPassingToDayjs(edu.yearOfPassing) || undefined}
                      onChange={(date, dateString) => {
                        const newEducation = [...(formData.education || [])];
                        newEducation[index].yearOfPassing = formatYearOfPassingForStore(date);
                        setFormData({ ...formData, education: newEducation });
                      }}
                  ...
                  <Card key={index} className="mb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <Form.Item
                        ...
                        label="Duration From"
                        rules={[{ required: true, message: "Duration From is required" }]}
                      >
                        <DatePicker
                          className="w-full"
                          picker="date"
                          format="DD/MM/YYYY"
                          value={parseDateStringToDayjs(exp.durationFrom) || undefined}
                          onChange={(date: Dayjs | null) => {
                            const newExperience = [...(formData.experience || [])];
                            newExperience[index].durationFrom = formatYearOfPassingForStore(date);
                            setFormData({ ...formData, experience: newExperience });
                          }}
                      ...
                      >
                        <DatePicker
                          ...
                          picker="date"
                          format="DD/MM/YYYY"
                          value={parseDateStringToDayjs(exp.durationTo) || undefined}
                          onChange={(date: Dayjs | null) => {
                            const newExperience = [...(formData.experience || [])];
                            newExperience[index].durationTo = date
                              ? formatYearOfPassingForStore(date)
                              : "";
                            setFormData({ ...formData, experience: newExperience });
                          }}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                          <DatePicker
                            ...
                            picker="date"
                            format="DD/MM/YYYY"
                            value={parseDateStringToDayjs(course.startDate) || undefined}
                            onChange={(date: Dayjs | null) => {
                              const newCourses = [...(formData.courses || [])];
                        ...
                        >
                          <DatePicker
                            ...
                            picker="date"
                            format="DD/MM/YYYY"
                            value={parseDateStringToDayjs(course.completionDate) || undefined}
                            onChange={(date: Dayjs | null) => {
                              const newCourses = [...(formData.courses || [])];
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                          <DatePicker
                            ...
                            picker="date"
                            format="DD/MM/YYYY"
                            value={parseDateStringToDayjs(internship.durationFrom) || undefined}
                            onChange={(date: Dayjs | null) => {
                              const newInternships = [...(formData.internships || [])];
                        ...
                        >
                          <DatePicker
                            ...
                            picker="date"
                            format="DD/MM/YYYY"
                            value={parseDateStringToDayjs(internship.durationTo) || undefined}
                            onChange={(date: Dayjs | null) => {
                              const newInternships = [...(formData.internships || [])];
                  ...
                  if (validExperiences.length > 0) {
                    return validExperiences.map((exp, i) => (
                      <div key={i} className="mb-4 p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Duration: {exp.durationFrom ? formatDateDisplay(exp.durationFrom) : 'N/A'} to {exp.durationTo ? formatDateDisplay(exp.durationTo) : 'Present'}
                        </p>
