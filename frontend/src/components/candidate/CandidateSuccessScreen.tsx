...
const CandidateSuccessScreen: React.FC<CandidateSuccessScreenProps> = ({
  ...
  // Displayed credentials: updated from API credentialsSent so UI matches what was sent via email/WhatsApp
  const [displayedUserAccount, setDisplayedUserAccount] = useState(userAccountProp);
  const [sendingChannel, setSendingChannel] = useState<'email' | 'whatsapp' | null>(null);
  const [sendCredentials] = useSendCandidateCredentialsMutation();

  useEffect(() => {
    if (userAccountProp) setDisplayedUserAccount(userAccountProp);
  }, [userAccountProp]);

  const userAccount = displayedUserAccount;

  // Login URL: use current domain (localhost or production) so it always matches where the user is
  const getDisplayLoginUrl = () => {
    if (typeof window === 'undefined') return userAccount?.loginUrl || '/login';
    const fallback = `${window.location.origin}/login`;
    const fromAccount = userAccount?.loginUrl;
    if (!fromAccount || fromAccount.includes('your-domain.com')) return fallback;
    return fromAccount;
  };
  const displayLoginUrl = getDisplayLoginUrl();

  const handleCopyCredentials = () => {
    if (!userAccount) return;

    const emailToUse = candidateEmail || userAccount.email;
    const credentials = `Login URL: ${displayLoginUrl}
...
Password: ${userAccount.password || "Use your existing password"}`;
  ...
  const handleSendViaMail = async () => {
    ...
    try {
      const result = await sendCredentials({ candidateId, channel: 'email', loginUrl: displayLoginUrl }).unwrap();
      message.success(result.message || 'Credentials sent successfully via email');
      if (result.data?.credentialsSent) {
        setDisplayedUserAccount((prev) => prev ? { ...prev, ...result.data!.credentialsSent } : prev);
      }
    } catch (error: any) {
  ...
  const handleShareViaWhatsApp = async () => {
    ...
    try {
      const result = await sendCredentials({ candidateId, channel: 'whatsapp', loginUrl: displayLoginUrl }).unwrap();
      message.success(result.message || 'Credentials sent successfully via WhatsApp');
      if (result.data?.credentialsSent) {
        setDisplayedUserAccount((prev) => prev ? { ...prev, ...result.data!.credentialsSent } : prev);
      }
    } catch (error: any) {
      const errorMsg = error?.data?.error?.whatsappError || error?.data?.error?.message || 'Failed to send WhatsApp message';
      ...
      <Card className="max-w-2xl w-full">
        ...
        <CardContent className="space-y-6">
                    ...
                    {userAccount.password ? (
                      ...
                      <div className="p-3 bg-amber-50 border border-amber-200 rounded">
                        <p className="text-sm text-amber-800">
                          No password shown (e.g. existing account). Click <strong>Send via Mail</strong> or <strong>Share via WhatsApp</strong> to generate and send credentials to the candidate; the new password will appear here after sending.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    ...
                    {candidateId && (
                      ...
                      >
                        ...
                        {sendingChannel === 'email' ? 'Sending...' : 'Send via Mail'}
                      </Button>
                    )}
                    {/* <Button
                      variant="outline"
                    ...
                    {candidateId && (
                      ...
                      >
                        ...
                        {sendingChannel === 'whatsapp' ? 'Sending...' : 'Share via WhatsApp'}
                      </Button>
                    )}
                  </div>
                </>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            {showGoToLogin && displayLoginUrl && (
              <Button
                onClick={() => {
                  window.location.href = displayLoginUrl;
                }}
            ...
            {onClose && (
              <Button
                variant="outline"
                onClick={onClose}
                className={showGoToLogin && displayLoginUrl ? "w-full sm:flex-1" : "w-full"}
                size="lg"
              >
                Close
              </Button>
