...
const InterviewStatusTimeline: React.FC<any> = ({ timeline }) => {
  ...
  if (!timeline.stages) {
    return (
      <div className="p-4 bg-yellow-50 text-yellow-600 rounded border border-yellow-200">
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "active":
        return "bg-blue-500 text-white border-blue-500 shadow-lg"; // Active stage
      case "rejected":
        return "bg-red-500 text-white border-red-500";
      case "failed":
        return "bg-red-500 text-white border-red-500";
      default:
        return "bg-white text-gray-300 border-gray-200"; // Pending -> grey outline
    }
  };
  ...
  const getIcon = (stepId: string) => {
    ...
    if (stepId === "document_collection")
      return <FileText className="w-4 h-4" />; // Or specific icon
    if (stepId === "background_verification")
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      ...
      <CardContent>
        <div className="relative flex items-start justify-between w-full px-2 overflow-x-auto pb-4 custom-scrollbar">
          {/* Progress Bar Background */}
          <div
            className="absolute top-5 left-0 w-full h-[2px] bg-slate-100 -translate-y-1/2 z-0 mx-8"
          ...
          {stages.map((step: any, index: number) => {
            ...
            const isClickable =
              step.id === "generate_offer" && isActive && candidateId;
