...
const SalaryStructureForm = ({ staffId, staff, onSave }: SalaryStructureFormProps) => {
  ...
  const handleInputChange = (field: keyof SalaryStructureInputs, value: string | number) => {
    setFormData(prev => {
      ...
      // Basic Salary and DA must not be negative
      if (field === 'basicSalary' || field === 'dearnessAllowance') {
  ...
  const handleSave = async () => {
    ...
    // Dearness Allowance must not be negative
    if ((formData.dearnessAllowance ?? 0) < 0) {
      message.error('Dearness Allowance (DA) cannot be negative');
      return;
    }
