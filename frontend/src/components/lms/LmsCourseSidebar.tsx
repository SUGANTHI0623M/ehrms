...
export interface Material {
    ...
    subtitleUrl?: string;
    status?: string;
    /** PDF page count when available from API */
    pageCount?: number;
    /** Video/audio duration in seconds when available from API */
...
interface LmsCourseSidebarProps {
    ...
    // Optional Overrides
    courseTitle?: string;
    customLearnerInfo?: React.ReactNode;
}
...
const LmsCourseSidebar: React.FC<LmsCourseSidebarProps> = ({
    ...
    const [mainActiveKeys, setMainActiveKeys] = useState<string[]>(isAdmin ? ['overview', 'curriculum'] : ['learner-info', 'curriculum']);

    const title = course?.title || courseTitle || 'Untitled Course';
    const description = course?.description;
    const enrolledAt = course?.enrolledAt; // pass this down if needed or handle in parent
    ...
    const renderLearnerInfo = () => {
        if (customLearnerInfo) return customLearnerInfo;

        // Default Learner Info View
        return (
            <div className="space-y-4">
                <div>
                    <Text type="secondary" className="text-xs uppercase font-bold tracking-wider">Progress</Text>
                    <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-sm font-semibold text-gray-700 whitespace-nowrap shrink-0">
                            {progressPercentage}% / {lessons.length} {lessons.length === 1 ? 'Lesson' : 'Lessons'}
                        </span>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="flex flex-col h-full bg-[#fcfcfd] border-l border-gray-100 shadow-sm custom-sidebar-container overflow-hidden sticky top-0" style={{ height: 'calc(100vh - 64px)' }}>
            {/* Top Fixed Header */}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                {isAdmin ? (
                    /* Admin: curriculum-only playlist */
                    <div className="playlist-root">
                        <div className="playlist-header">
                            <span className="playlist-title">Course content</span>
                            <span className="playlist-meta">{TOTAL_ITEMS_LABEL(lessons.length, allMaterials.length)}</span>
                        </div>
                        <Collapse
                            accordion
                    ...
                    >
                        {/* 1. Learner Info Panel (Only for Employee or if custom info provided) */}
                        {(!isAdmin || customLearnerInfo) && (
                            <Panel
            ...
            {sidebarFooter && (
                <div className="p-4 border-t border-gray-100 bg-white z-10 shadow-[0_-5px_15px_rgba(0,0,0,0.02)]">
                    {sidebarFooter}
                </div>
            )}
            ...
            <style>{`
                ...
                .playlist-accordion.ant-collapse-ghost > .ant-collapse-item > .ant-collapse-content > .ant-collapse-content-box {
                    padding: 0 0 12px 0 !important;
                }
                .playlist-accordion .ant-collapse-header {
                ...
                .playlist-panel .ant-collapse-content-box {
