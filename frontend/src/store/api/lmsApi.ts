...
export const lmsApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getEmployees: builder.query<{ success: boolean; data: { staff: any[] } }, void>({
      query: () => ({
        url: '/staff',
        params: { limit: 50, status: 'Active', page: 1 }
      }),
    }),
    ...
    getCourseAssessmentAttemptReport: builder.query<
      { success: boolean; data: any },
      { courseId: string; employeeId: string }
    >({
      query: ({ courseId, employeeId }) => ({
        url: '/lms/assessment/course-attempt-report',
        params: { courseId, employeeId },
      }),
      providesTags: (_result, _err, { courseId, employeeId }) =>
        [{ type: 'LMS_ASSESSMENT_RESULTS' as any, id: `attempt-${courseId}-${employeeId}` }],
    }),

    getCourseAssessmentAttemptReportById: builder.query<
      { success: boolean; data: any },
      string
    >({
      query: (attemptId) => ({
        url: `/lms/assessment/attempt-report/${attemptId}`,
      }),
      providesTags: (_result, _err, attemptId) =>
        [{ type: 'LMS_ASSESSMENT_RESULTS' as any, id: `attempt-by-id-${attemptId}` }],
    }),

    getMyCourseAssessmentAttemptReport: builder.query<
      { success: boolean; data: any },
      { courseId: string }
    >({
      query: ({ courseId }) => ({
        url: '/lms/assessment/my-attempt-report',
        params: { courseId },
      }),
      providesTags: (_result, _err, { courseId }) =>
        [{ type: 'LMS_ASSESSMENT_RESULTS' as any, id: `my-attempt-${courseId}` }],
    }),
    ...
    updateAssessmentRemarks: builder.mutation<
        { success: boolean; data: { assessmentRemarks: string } },
        { courseId: string; remarks: string }
    >({
      query: ({ courseId, remarks }) => ({
        url: `/lms/courses/${courseId}/assessment/remarks`,
        method: 'PATCH',
        body: { remarks }
      }),
      invalidatesTags: ['LMS', 'LMS_ASSESSMENT_RESULTS' as any],
...
export const {
  ...
  useGetCourseAssessmentAttemptReportQuery,
  useGetCourseAssessmentAttemptReportByIdQuery,
  useGetMyCourseAssessmentAttemptReportQuery,
