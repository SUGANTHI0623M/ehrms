...
export interface CreateStaffRequest {
  employeeId?: string; // Optional; backend auto-generates if not provided
  name: string;
  email: string;
  phone: string;
  countryCode?: string;
  ...
  joiningDate?: string; // Joining date in ISO format (YYYY-MM-DD)
  shiftName?: string;
  attendanceTemplateId?: string;
  leaveTemplateId?: string;
  holidayTemplateId?: string;
...
export const staffApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    importStaffFromExcel: builder.mutation<
      {
        ...
        data: {
          ...
          failedItems: Array<{ row: number; name: string; email: string; error: string }>;
        };
      },
      { file: File }
    >({
    ...
    toggleStaffTwoFactor: builder.mutation<
      { success: boolean; message: string; data: { staffId: string; twoFactorEnabled: boolean } },
      { staffId: string; enabled: boolean }
    >({
      query: ({ staffId, enabled }) => ({
        url: `/staff/${staffId}/two-factor`,
        method: 'PATCH',
        body: { enabled },
      }),
      invalidatesTags: (result, error, { staffId }) => [
...
export const {
  ...
  useToggleStaffTwoFactorMutation,
