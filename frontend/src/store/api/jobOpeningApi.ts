...
export const jobOpeningApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getJobOpeningDashboard: builder.query<
      {
        success: boolean;
        data: {
          ...
          openingsByRole: Array<{ role: string; count: number }>;
          jobOpenings: Array<{
            _id: string;
            title: string;
            status: string;
      ...
      void
    >({
      query: () => '/job-openings/dashboard',
      providesTags: ['JobOpening', 'Candidate'],
    }),
    ...
    >({
      query: (data) => ({
        url: '/job-openings/generate-description',
        method: 'POST',
        body: data,
      }),
    }),
