    ...
    sendCandidateCredentials: builder.mutation<
      { 
        ...
        data: { 
          ...
          /** When present, UI should show these so displayed credentials match what was sent */
          credentialsSent?: { loginUrl: string; email: string; password: string };
        } 
      },
      { candidateId: string; channel?: 'email' | 'whatsapp' | 'both'; loginUrl?: string }
    >({
      query: ({ candidateId, channel, loginUrl }) => {
        // Ensure candidateId is always a string (avoid "[object Object]" when an object is passed)
        const id =
          typeof candidateId === 'string'
            ? candidateId
            : candidateId && typeof candidateId === 'object'
              ? String((candidateId as { _id?: string; id?: string })._id ?? (candidateId as { _id?: string; id?: string }).id ?? '')
              : String(candidateId ?? '');
        const body: { channel?: string; loginUrl?: string } = {};
        if (channel) body.channel = channel;
        if (loginUrl) body.loginUrl = loginUrl;
        return {
          url: `/candidate-form/${id}/send-credentials`,
          method: 'POST',
          body,
        };
      },
