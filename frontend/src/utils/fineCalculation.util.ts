...
export interface AttendanceRecord {
  ...
  fineAmount?: number;
  status?: string;
  halfDaySession?: 'First Half Day' | 'Second Half Day'; // Half-day leave session
  fineHours?: number; // Total fine hours in minutes
  earlyMinutes?: number; // Early exit minutes
}

/**
 * Calculate total fine from attendance records
 * Excludes fines for half-day leaves where employee logged in correctly
 */
export function calculateTotalFine(attendanceRecords: AttendanceRecord[]): {
  ...
  attendanceRecords.forEach(record => {
    // Skip fines for half-day leaves where employee logged in correctly
    // If halfDaySession is set and fineAmount is 0 or very small (due to correct login), exclude it
    if (record.halfDaySession) {
      // For half-day leaves, only count fine if it's significant (employee was actually late)
      // If fineAmount is 0 or very small, it means employee logged in correctly
      if (record.fineAmount && record.fineAmount > 0.01) {
        // Employee was late even for half-day, count the fine
      ...
      // If fineAmount is 0 or very small, employee logged in correctly - don't count fine
    } else {
      // Regular attendance - count fine if present
