...
export function calculateWorkingDays(
  ...
  const lastDay = endDate ? new Date(endDate) : new Date(year, month + 1, 0);
  lastDay.setHours(23, 59, 59, 999); // Set to end of day
  
  // Ensure endDate is within the same month
  if (endDate && (endDate.getMonth() !== month || endDate.getFullYear() !== year)) {
    // If endDate is in a different month, use end of the specified month
  ...
  // Filter holidays to only count those within the date range
...
/** Format currency (INR) — human-readable with Indian grouping and ₹ symbol */
