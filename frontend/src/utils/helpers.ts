/**
 * Normalize search input so pasted text (with newlines/extra spaces) filters correctly.
 * Trims and collapses any run of whitespace (including newlines) to a single space.
 * Use in search input onChange so typing and paste behave the same.
 */
...
/**
 * Normalized search string for filtering (case-insensitive match).
 * Use when applying filter so pasted content matches the same as typed.
 */
export const getNormalizedSearchForFilter = (value: string): string => {
