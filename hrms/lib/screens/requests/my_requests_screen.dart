...
class MyRequestsScreen extends StatefulWidget {
  ...
  /// When true, this screen is the visible tab (e.g. user tapped Request in bottom nav). Used to refetch payslip setting.
...
class _MyRequestsScreenState extends State<MyRequestsScreen>
  ...
  /// When true, payslips are auto-generated; hide Payslip tab (user downloads from Salary).
  /// Start false so Payslip tab is visible by default; hide only when API returns true.
  bool _isPayslipAutoGenerated = false;

  /// True while fetching isPayslipAutoGenerated; show circular loader until done.
  bool _isLoadingPayslipSetting = true;
  ...
  Future<void> _loadPayslipSetting() async {
    if (!mounted) return;
    setState(() => _isLoadingPayslipSetting = true);
    try {
      final result = await AttendanceService().getFineCalculation();
      if (!mounted) return;
      if (result['success'] != true) {
        if (mounted) setState(() => _isLoadingPayslipSetting = false);
        return;
      }
      final payslip = result['payslip'];
      final isAuto =
          payslip is Map && (payslip['isPayslipAutoGenerated'] == true);
      if (_isPayslipAutoGenerated != isAuto) {
        final oldIndex = _tabController.index;
        final count = isAuto ? 3 : 4;
        if (!mounted) return;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          setState(() {
            _isPayslipAutoGenerated = isAuto;
            _isLoadingPayslipSetting = false;
          });
        });
        return;
      }
    ...
    return Scaffold(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          if (!_isPayslipAutoGenerated) PayslipRequestsTab(key: _payslipTabKey),
        ],
      ),
  ...
  Widget? _buildFab() {
    ...
    switch (_tabController.index) {
      ...
      case 3: // Payslip (only when tab is visible, i.e. !_isPayslipAutoGenerated)
        return SizedBox(
          height: 40,
          child: FloatingActionButton.extended(
            foregroundColor: Colors.white,
